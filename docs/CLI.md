...
| `client` | `client/shim.mjs` | Run the local HTTP-CONNECT proxy (fleet client). Long-running. Finds which set holds your leaf (`members.json`, then each `--group-contract`, then `--paid-access-contract`; `--leaf-source` pins one) and proves against it; `--limit` must be your tier. Routes only to gateways whose signed policy admits that leaf source; `--max-anon` = invited-only gateways only (refuses a staked/paid leaf with the reason). A named network works only when its record is current and non-retired; the committed Sepolia profile is not v4-compatible. | `shade-tree client --secret <hex> --bootnode <v4-bootnode.onion> --dir-signer <v4-signer-hex>` (paid: also pass `--leaf-source paid --limit 32 --paid-access-contract <v4-paid-set>`; maximum anonymity: add `--max-anon`) |
