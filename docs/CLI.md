...
| `gateway` | `gateway/gateway.mjs` | Run the reputation-gated egress gateway (Tor onion, `:443` metadata-only tunnel). Long-running. Trusts the UNION of its root sources (T-FEAT-7): `members.json` + every `--group-contract` (comma list) + `--paid-access-contract`; `--roots static,onchain` picks explicitly. | `rgoe gateway --group-contract 0xSet --paid-access-contract 0xPaid --rpc-url https://rpc.example` |
| `client` | `client/shim.mjs` | Run the local HTTP-CONNECT proxy (fleet client). Long-running. Finds which set holds your leaf (`members.json`, then each `--group-contract`, then `--paid-access-contract`) and proves against it; `--limit` must be your tier. | `rgoe client --secret <hex> --bootnode <onion> --dir-signer <hex>` (or `--network sepolia`; a paid member: `--network sepolia --limit 32`) |
...
| `--roots` | `RGOE_ROOTS` | gateway (`static,onchain`; T-FEAT-7) |
