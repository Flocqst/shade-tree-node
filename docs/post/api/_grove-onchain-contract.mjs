...
function validCountMetric(value, key, minimumCohort) {
...
export function validPublicOnchain(value, observedAt) {
  ...
  if (!exactKeys(value, ["definition", "generatedAt", "delayHours", "minimumCohort", "source", "membership", "settlements", "enforcement"])
    ...
    || decimalU64(value.source.finalizedBlock) === null || !HASH.test(value.source.finalizedBlockHash)
    || !Number.isFinite(finalizedAt) || finalizedAt > generatedAt - value.delayHours * 60 * 60_000
    ...
    || !validCountMetric(value.membership.staked, "activeCommitments", value.minimumCohort)
    || !validCountMetric(value.membership.paid, "activeCommitments", value.minimumCohort)
    || !exactKeys(value.enforcement, ["definition", "staked", "paid"])
    || value.enforcement.definition !== "finalized-contract-slash-events"
    || !validCountMetric(value.enforcement.staked, "finalizedSlashes", value.minimumCohort)
    || !validCountMetric(value.enforcement.paid, "finalizedSlashes", value.minimumCohort)) return false;
  ...
  return settlement.status === "available"
    ...
    && settlement.asset.chainId === value.source.chainId && address(settlement.asset.address)
...
export function publicOnchainSigningPayload(value) {
  return {
