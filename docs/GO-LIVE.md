...
- **GAP-1** — (OPEN → resolved by the go-live log: droplet-1 = the idle June PoC box `anon-egress`, droplet-2 = agent-devops `rgoe-gw-04`.) No host named `anon-egress` exists in this repo or `~/agent-devops` (only the `anon_egress` firewall/role class covering `egress-01`, `egress-02`, `rgoe-03`, plus an older PoC gateway referenced by IP in `docs/JOIN.md`/`scripts/run-client.sh`). SHIP-PLAN T-DEPLOY-1 should name the box(es) explicitly.
