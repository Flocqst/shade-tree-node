...
| 3.6 | Heartbeat signs operator auth | droplet-1 | Add `Environment=SHADE_TREE_GW_OPERATOR_KEY=<operator-key>` to `shade-tree-heartbeat.service` (or `SHADE_TREE_GW_OPERATOR` + precomputed `SHADE_TREE_GW_OPERATOR_SIG` to keep the key off-box); `systemctl daemon-reload && systemctl restart shade-tree-heartbeat` | The `heartbeat accepted` JSON record has `"staked":true`; the `/directory` entry shows staked. | Remove var, restart; bootnode back to `open` (3.5 rollback). |
