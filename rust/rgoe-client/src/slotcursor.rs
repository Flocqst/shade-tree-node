...
//! client's `makeSlotPool` (client/rgoe-client.mjs).
//!
...
//! `rgoe egress` is a ONE-SHOT CLI: without persistence every invocation would reuse
//! slot 0 and mint the SAME nullifier, wasting the K-1 other slots. This module keeps
//! a tiny on-disk cursor so consecutive `rgoe egress` runs within one epoch advance
...
/// `export const K_SLOTS = Number(process.env.RGOE_SLOTS || 8)` (line 66) — the app
...
/// JS `makeSlotPool().nextSlot()` (client/rgoe-client.mjs): hand out the cursor, then
