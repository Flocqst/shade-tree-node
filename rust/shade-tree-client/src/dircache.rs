...
//! discipline (T-RUST-3, slice 2). Default-build (serde_json + shade-tree-proto only).
//!
...
// input: we deserialize it, map it into shade-tree-proto's Directory, and let
...
/// mapped to shade_tree_proto's `Caps` and CANONICALIZED (dedup/sort/range-check) by the proto
...
pub struct CapsDto {
    ...
    // T-HARD-8: accepted ZK artifact ids (canonicalized by shade-tree-proto; junk dropped there).
    #[serde(default)]
    pub artifacts: Option<Vec<String>>,
    // T-FEAT-9: admission policy + payment advert (canonicalized by shade-tree-proto; junk dropped).
    #[serde(default)]
...
impl PayCapsDto {
    fn into_proto(self) -> shade_tree_proto::PayCaps {
