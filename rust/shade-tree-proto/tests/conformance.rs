...
//! `shade-tree-proto` reproduce every pinned value exactly. Where a value is byte-pinned
...
fn receipt_domain_matches_vector() {
    let v = vectors();
    assert_eq!(
        shade_tree_proto::RECEIPT_DOMAIN,
...
fn proto_reason_labels_match_vector() {
    ...
    let range = (shade_tree_proto::PROTO_MIN, shade_tree_proto::PROTO_MAX);
...
fn artifact_id_matches_vector() {
    ...
    assert_eq!(
        shade_tree_proto::artifact_id_of(s(smp, "circuit"), s(smp, "inputUtf8").as_bytes()),
        s(smp, "artifactId")
    );
    assert!(shade_tree_proto::is_artifact_id(s(smp, "artifactId")));
    assert!(!shade_tree_proto::is_artifact_id("Not An Id"));
    assert!(!shade_tree_proto::is_artifact_id(""));
    assert!(!shade_tree_proto::is_artifact_id(&"a".repeat(65)));
    assert!(shade_tree_proto::is_artifact_id(&"a".repeat(64)));
}
...
fn artifact_reason_labels_match_vector() {
    ...
    assert_eq!(shade_tree_proto::REASON_BAD_ARTIFACT, s(r, "badArtifact"));
    assert_eq!(
        shade_tree_proto::REASON_ARTIFACT_RETIRED,
        s(r, "artifactRetired")
    );
    assert_eq!(
        shade_tree_proto::REASON_ARTIFACT_UNKNOWN,
        s(r, "artifactUnknown")
    );
    assert_eq!(
        shade_tree_proto::REASON_NO_MUTUAL_ARTIFACT,
        s(r, "noMutualArtifact")
    );
    assert_eq!(
        shade_tree_proto::REASON_NO_CLIENT_ARTIFACT,
...
fn select_artifact_matches_vector() {
    ...
    assert_eq!(
        shade_tree_proto::select_artifact(Some(&ad), &mine).unwrap(),
        s(sel, "picked")
    );
    assert_eq!(
        shade_tree_proto::select_artifact(None, &mine).unwrap(),
        s(sel, "noAdPicked")
    );
    assert_eq!(
        shade_tree_proto::select_artifact(Some(&[]), &mine).unwrap(),
        s(sel, "noAdPicked"),
    ...
    let err = shade_tree_proto::select_artifact(Some(&disjoint), &mine).unwrap_err();
    assert_eq!(err.to_string(), s(sel, "disjointReason"));
    let err = shade_tree_proto::select_artifact(None, &[]).unwrap_err();
    assert_eq!(
        err.to_string(),
        s(&v["artifacts"]["reasons"], "noClientArtifact")
    );
    // junk client ids are ignored, not selected
    let err = shade_tree_proto::select_artifact(None, &["Not An Id".to_string()]).unwrap_err();
    assert_eq!(err.to_string(), shade_tree_proto::REASON_NO_CLIENT_ARTIFACT);
}
...
fn caps_with_artifacts_match_vector() {
    ...
    let cc = shade_tree_proto::canonical_caps(&caps);
    assert_eq!(
        cc.artifacts.as_deref().unwrap(),
        str_vec(&cwa["canonical"]["artifacts"])
    );
    assert!(
        shade_tree_proto::has_caps(&Caps {
    ...
    assert_eq!(shade_tree_proto::canonical_caps(&junk).artifacts, None);
    let too_many: Vec<String> = (0..=shade_tree_proto::MAX_CAPS_ARTIFACTS)
        .map(|i| format!("rln-{i:016x}"))
        .collect();
    assert_eq!(
        shade_tree_proto::canonical_caps(&Caps {
...
fn vector_pay(p: &Value) -> shade_tree_proto::PayCaps {
...
fn caps_with_admission_match_vector() {
    ...
    assert_eq!(
        ...
        shade_tree_proto::PAY_PROTOCOLS
            .iter()
            .map(|x| x.to_string())
            .collect::<Vec<_>>()
    );
    assert_eq!(
        ...
        shade_tree_proto::MAX_CAPS_PAY_TIERS
    );
    let caps = Caps {
        ports: Some(str_ports(&cwa["caps"]["ports"])),
        proto: Some(ProtoCaps {
    ...
    assert!(shade_tree_proto::has_caps(&Caps {
