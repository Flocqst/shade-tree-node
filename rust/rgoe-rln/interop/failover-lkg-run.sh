...
# (client/rgoe-client.mjs parity) — GATEWAY FAILOVER and LAST-KNOWN-GOOD directory
# caching — against the REAL JS gateway, with NO Tor (plain-TCP, always-green like
# egress-run.sh). Two layers:
#
#   LAYER A (FAILOVER): `rgoe egress` is given an ORDERED candidate list whose FIRST
...
#     retry). Mirrors the connect() failover loop in client/rgoe-client.mjs.
#
#   LAYER B (LAST-KNOWN-GOOD): `rgoe fetch-directory` verifies a fresh signed
...
#     is kept. Then `rgoe select` picks a gateway from the cache. Mirrors
...
# are exercised by egress-tor-run.sh under RGOE_TOR_E2E; this plain-TCP harness is the
# authoritative always-green check, exactly like egress-run.sh.
#
# Prereqs: `npm install` at the repo root (rlnjs) + a Rust toolchain.
# Run from anywhere: bash rust/rgoe-rln/interop/failover-lkg-run.sh
set -euo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO="$(cd "$HERE/../../.." && pwd)"
CIRCUITS="$REPO/circuits/rln"
TESTDATA="$REPO/rust/rgoe-client/src/testdata"
WORK="$(mktemp -d)"
...
OUT="$($RGOE fetch-directory --file "$TESTDATA/lkg_dir_200.json" --signer "$SIGNER" --cache "$CACHE")"
echo "$OUT"
...
OUT="$($RGOE fetch-directory --file "$WORK/does-not-exist.json" --signer "$SIGNER" --cache "$CACHE")"
echo "$OUT"
...
OUT="$($RGOE fetch-directory --file "$TESTDATA/lkg_dir_100.json" --signer "$SIGNER" --cache "$CACHE")"
echo "$OUT"
...
OUT="$($RGOE select "$CACHE" --signer "$SIGNER" --seed 1)"
...
set +e
"$RGOE" egress --plain-tcp "127.0.0.1:${DEAD_PORT},127.0.0.1:${GW_PORT}" \
  --identity "$IDENTITY" --members "$MEMBERS" --target "$TARGET" \
  --circuits "$CIRCUITS" > "$WORK/rgoe.out" 2> "$WORK/rgoe.err"
A_RGOE_RC=$?
set -e
node "$HERE/wait-log.mjs" "$WORK/gw.log" "egress target=${TARGET} " 5000 >/dev/null 2>&1 || true
echo "--- rgoe stderr (failover trace) ---"; cat "$WORK/rgoe.err"
echo "--- rgoe stdout ---"; cat "$WORK/rgoe.out"
...
grep -q "candidate 127.0.0.1:${DEAD_PORT} failed" "$WORK/rgoe.err" || { echo "LAYER A FAILED: no rotation off the dead candidate"; exit 1; }
grep -q "^ok" "$WORK/rgoe.out" || { echo "LAYER A FAILED: client did not report ok"; exit 1; }
[ "$A_RGOE_RC" -eq 0 ] || { echo "LAYER A FAILED: rgoe rc=$A_RGOE_RC"; exit 1; }
