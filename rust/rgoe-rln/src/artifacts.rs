...
//! also derives the set's artifact id (`rgoe_proto::artifact_id_of("rln", vkey_bytes)`,
...
/// them. Called at `rgoe egress` startup (embedded path); a mismatch is a hard, named error.
#[cfg(feature = "embedded-artifacts")]
...
mod tests {
    ...
    fn tree_artifacts_match_lock_and_id() {
        ...
        assert_eq!(c.artifact_id.len(), 4 + rgoe_proto::ARTIFACT_ID_HASH_CHARS);
