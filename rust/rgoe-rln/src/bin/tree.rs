//! `rgoe-rln-tree` — native Rust merkle root/path emitter (T-RUST-2c).
//!
...
//!
//! Usage:
//!   rgoe-rln-tree root  [--id ID] <leaf...>
//!       -> prints the tree root (decimal) for the ordered member list.
//!   rgoe-rln-tree proof [--id ID] <index> <leaf...>
...
//!
//! `--id` is the group id / RLN identifier (default 1, matching RGOE_RLN_IDENTIFIER=1).

use num_bigint::BigUint;
use rgoe_rln::tree::{dec_to_fr, fr_to_dec, MerkleTree, TREE_DEPTH};
...
fn main() {
    ...
    if args.is_empty() {
        eprintln!("usage: rgoe-rln-tree <root|proof> [--id ID] ...");
        std::process::exit(2);
    }
    ...
    match mode.as_str() {
        "root" => {
            if args.is_empty() {
                eprintln!("usage: rgoe-rln-tree root [--id ID] <leaf...>");
                std::process::exit(2);
            }
        ...
        "proof" => {
            if args.len() < 2 {
                eprintln!("usage: rgoe-rln-tree proof [--id ID] <index> <leaf...>");
                std::process::exit(2);
            }
