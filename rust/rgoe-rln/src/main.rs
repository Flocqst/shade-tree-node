...
//!    conformance-gated code the JS reference uses (`rgoe_proto::calculate_signal_hash`
//!    over `rgoe_proto::request_signal`), so the Rust side owns the target binding.
...
//!
//! Usage: `rgoe-rln-probe <fixture.json> <out.json> <circuits-dir>`
...
fn main() {
    ...
    if args.len() != 3 {
        eprintln!("usage: rgoe-rln-probe <fixture.json> <out.json> <circuits-dir>");
        std::process::exit(2);
    }
    ...
    //     with the conformance-gated rgoe-proto code and assert it matches the fixture.
    let signal = rgoe_proto::request_signal(&fixture.target, &fixture.nonce);
    let x_rust = rgoe_proto::calculate_signal_hash(&signal);
    assert_eq!(
        x_rust, fixture.x,
