...
# Run from anywhere: bash rust/shade-tree-rln/interop/tree-run.sh
set -euo pipefail
...
cargo build -p shade-tree-rln --bin shade-tree-rln-probe --bin shade-tree-rln-tree --manifest-path "$REPO/rust/Cargo.toml"
PROBE="$REPO/rust/target/debug/shade-tree-rln-probe"
TREE="$REPO/rust/target/debug/shade-tree-rln-tree"
