...
//! ```
//!
//! Group id / RLN identifier is 1 (SHADE_TREE_RLN_IDENTIFIER default), tree depth 20.

use ark_bn254::Fr;
use num_bigint::BigUint;
use shade_tree_rln::tree::{dec_to_fr, fr_to_dec, poseidon2, MerkleTree, TREE_DEPTH};
...
fn rate_commitment_tiers_match_js_derive_commitment() {
    ...
    let s = dec_to_fr("111");
    assert_eq!(
        fr_to_dec(&shade_tree_rln::tree::rate_commitment(s, 8)),
    ...
    assert_eq!(
        fr_to_dec(&shade_tree_rln::tree::rate_commitment(s, 32)),
        "15363698809722346745616993869789510363416645981863858152379739283427647190637",
        "limit-32 leaf == JS deriveCommitment(111n, 32)"
    );
    assert_ne!(
        shade_tree_rln::tree::rate_commitment(s, 8),
        shade_tree_rln::tree::rate_commitment(s, 32),
