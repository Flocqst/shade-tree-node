...
output "provisioning_log_command" {
  ...
  value       = "ssh root@${digitalocean_droplet.bootnode.ipv4_address} 'tail -f /var/log/rgoe-bootstrap.log'"
}
...
output "verify_command" {
  ...
  value = join(" && ", [
    "ssh root@${digitalocean_droplet.bootnode.ipv4_address} 'cat /opt/rgoe/deploy-state/bootnode-hs/hostname'",
    "echo 'then, over Tor SOCKS ${var.tor_socks_port}:'",
    "ssh root@${digitalocean_droplet.bootnode.ipv4_address} 'BN=$(cat /opt/rgoe/deploy-state/bootnode-hs/hostname); curl --socks5-hostname 127.0.0.1:${var.tor_socks_port} http://$BN/health'"
  ])
}

output "rolling_update_hint" {
  description = "Move this box to a new git ref later without re-provisioning (T-DEPLOY-6)."
  value       = "ssh root@${digitalocean_droplet.bootnode.ipv4_address} 'sudo bash /opt/rgoe/bootnode/deploy/rolling-update.sh <ref>'"
}
