...
//      (neither var, and ethers is never even imported), RGOE_GW_OPERATOR_KEY (signs
//      operatorAuthMessage — byte-matched against the pinned testdata/vectors.json
//      operatorAnnounce vector, 0x-optional, whitespace-tolerant), a pre-computed
//      RGOE_GW_OPERATOR + RGOE_GW_OPERATOR_SIG pair (verified locally; wins over the key), and
...
async function main() {
  ...
  ok(JSON.stringify(await resolveOperator(V.onion, { RGOE_UNRELATED: "x" })) === JSON.stringify(none), "unrelated env -> still onion-only");

  // Key path, byte-matched against the pinned operatorAnnounce vector.
  const fromKey = await resolveOperator(V.onion, { RGOE_GW_OPERATOR_KEY: OA.operatorPriv }, { importEthers: countingEthers });
  ok(ethersImports === 1, "key path imports ethers (lazily, once)");
  ok(fromKey.operator === OA.operator, "RGOE_GW_OPERATOR_KEY -> operator address == pinned vector operator");
  ok(fromKey.operatorSig === OA.operatorSig, "RGOE_GW_OPERATOR_KEY -> operatorSig BYTE-MATCHES the pinned personal_sign vector");
  ...
  const noPrefix = await resolveOperator(V.onion, { RGOE_GW_OPERATOR_KEY: OA.operatorPriv.slice(2) });
  ok(noPrefix.operatorSig === OA.operatorSig, "key without 0x prefix -> same sig");
  const padded = await resolveOperator(V.onion, { RGOE_GW_OPERATOR_KEY: `  ${OA.operatorPriv}\n` });
  ...
  const otherSig = await resolveOperator(other.onion, { RGOE_GW_OPERATOR_KEY: OA.operatorPriv });
  ...
  ethersImports = 0;
  const pair = await resolveOperator(V.onion, { RGOE_GW_OPERATOR: OA.operator, RGOE_GW_OPERATOR_SIG: OA.operatorSig }, { importEthers: countingEthers });
  ok(pair.operator === OA.operator && pair.operatorSig === OA.operatorSig, "RGOE_GW_OPERATOR + _SIG pair -> returned as given (verified locally)");
  ok(ethersImports === 0, "pair path never touches the Wallet path (no key needed on the box)");
  const lower = await resolveOperator(V.onion, { RGOE_GW_OPERATOR: OA.operator.toLowerCase(), RGOE_GW_OPERATOR_SIG: OA.operatorSig });
  ...
  const prec = await resolveOperator(V.onion, { RGOE_GW_OPERATOR: OA.operator, RGOE_GW_OPERATOR_SIG: OA.operatorSig, RGOE_GW_OPERATOR_KEY: otherKey });
  ok(prec.operator === OA.operator && prec.operatorSig === OA.operatorSig, "precedence: pre-computed pair WINS over RGOE_GW_OPERATOR_KEY");
  // Key + only half a pair -> the key path (the half pair is ignored because the key is authoritative).
  const keyHalf = await resolveOperator(V.onion, { RGOE_GW_OPERATOR: OA.operator, RGOE_GW_OPERATOR_KEY: OA.operatorPriv });
  ...
  ok((await throwsWith(() => resolveOperator(V.onion, { RGOE_GW_OPERATOR: OA.operator }), /must be set together/)).threw, "operator WITHOUT sig (no key) -> startup error, not silent onion-only");
  ok((await throwsWith(() => resolveOperator(V.onion, { RGOE_GW_OPERATOR_SIG: OA.operatorSig }), /must be set together/)).threw, "sig WITHOUT operator -> startup error");
  ok((await throwsWith(() => resolveOperator(V.onion, { RGOE_GW_OPERATOR: "dEaD", RGOE_GW_OPERATOR_SIG: OA.operatorSig }), /RGOE_GW_OPERATOR is not/)).threw, "malformed operator address -> error");
  ok((await throwsWith(() => resolveOperator(V.onion, { RGOE_GW_OPERATOR: OA.operator, RGOE_GW_OPERATOR_SIG: "0xabc" }), /RGOE_GW_OPERATOR_SIG is not/)).threw, "malformed operator sig -> error");
  ok((await throwsWith(() => resolveOperator(other.onion, { RGOE_GW_OPERATOR: OA.operator, RGOE_GW_OPERATOR_SIG: OA.operatorSig }), /does not recover/)).threw, "pinned sig presented for a DIFFERENT onion -> does-not-recover error (sig is onion-bound)");
  ok((await throwsWith(() => resolveOperator(V.onion, { RGOE_GW_OPERATOR: V.operator, RGOE_GW_OPERATOR_SIG: OA.operatorSig }), /does not recover/)).threw, "pinned sig with a DIFFERENT operator address -> does-not-recover error");
  const flipped = OA.operatorSig.slice(0, -2) + (OA.operatorSig.endsWith("1c") ? "1b" : "1c");
  ok((await throwsWith(() => resolveOperator(V.onion, { RGOE_GW_OPERATOR: OA.operator, RGOE_GW_OPERATOR_SIG: flipped }), /does not recover/)).threw, "sig with flipped recovery byte -> does-not-recover error");
  ...
  for (const [label, key] of Object.entries(badKeys)) {
    const r = await throwsWith(() => resolveOperator(V.onion, { RGOE_GW_OPERATOR_KEY: key }), /RGOE_GW_OPERATOR_KEY is not/);
  ...
  const rEcho = await throwsWith(() => resolveOperator(V.onion, { RGOE_GW_OPERATOR_KEY: OA.operatorPriv }, { importEthers: echoingEthers }), /not a valid secp256k1/);
  ...
  // ===================================================================================
  console.log("\n2. announce construction + signing (byte-match vs testdata/vectors.json):");
  ...
  ok(r.ok === true && announced === 1 && probed === 0, "RGOE_EGRESS_CHECK=0 -> announces unconditionally, probe not even called");
  ...
  ok((await throwsWith(async () => heartbeatConfig({}), /set RGOE_BOOTNODE_ONION/)).threw, "no RGOE_BOOTNODE_ONION -> fail fast");
  const dc = heartbeatConfig({ RGOE_BOOTNODE_ONION: "b.onion" });
  ok(dc.intervalSec === 300 && dc.weight === 100 && dc.torHost === "127.0.0.1" && dc.torPort === 9250, "defaults: 300s, weight 100, 127.0.0.1:9250");
  const oc = heartbeatConfig({ RGOE_BOOTNODE_ONION: "b.onion", RGOE_BOOTNODE_HEARTBEAT: "60", RGOE_GW_WEIGHT: "7", RGOE_TOR_HOST: "10.0.0.2", RGOE_TOR_PORT: "9050" });
  ok(oc.intervalSec === 60 && oc.weight === 7 && oc.torHost === "10.0.0.2" && oc.torPort === 9050, "overrides parsed");
  ok(egressCheckEnabled({}) === true && egressCheckEnabled({ RGOE_EGRESS_CHECK: "0" }) === false && egressCheckEnabled({ RGOE_EGRESS_CHECK: "1" }) === true, "egressCheckEnabled: default ON, `0` OFF");
  ...
  const id = await loadIdentity({ RGOE_GW_IDENTITY: "/id/good.json" }, { readFile: fakeRead });
  ok(id.onion === V.onion && id.seed === V.onionSeed, "loadIdentity reads RGOE_GW_IDENTITY path -> {onion, seed}");
  ok((await throwsWith(() => loadIdentity({ RGOE_GW_IDENTITY: "/id/noseed.json" }, { readFile: fakeRead }), /missing onion\/seed/)).threw, "identity missing seed -> error naming keygen");
  ok((await throwsWith(() => loadIdentity({ RGOE_GW_IDENTITY: "/id/arr.json" }, { readFile: fakeRead }), /missing onion\/seed/)).threw, "identity that is not an object -> error");
  ok((await throwsWith(() => loadIdentity({ RGOE_GW_IDENTITY: "/id/bad.json" }, { readFile: fakeRead }), /JSON/)).threw, "malformed identity JSON -> error");
  ok((await throwsWith(() => loadIdentity({ RGOE_GW_IDENTITY: "/id/nope.json" }, { readFile: fakeRead }), /ENOENT/)).threw, "missing identity file -> ENOENT surfaces");
  let defaultPath = "";
  await loadIdentity({}, { readFile: async (p) => { defaultPath = p; return JSON.stringify(VID); } });
  ok(defaultPath.endsWith(join("tor", "hs", "identity.local.json")), "no RGOE_GW_IDENTITY -> default tor/hs/identity.local.json");

  // ===================================================================================
  console.log("\n5. runHeartbeat with a fake scheduler: interval, outage recovery, wiring:");
  const gw = newOnion();
  const opKey = "0x" + randomBytes(32).toString("hex");
  const env = { RGOE_BOOTNODE_ONION: V.onion, RGOE_BOOTNODE_HEARTBEAT: "7", RGOE_GW_WEIGHT: "55", RGOE_TOR_HOST: "127.0.0.3", RGOE_TOR_PORT: "9150",
    RGOE_GW_IDENTITY: "/gw/identity.local.json", RGOE_GW_OPERATOR_KEY: opKey, RGOE_EGRESS_ALLOW: "*:443,*:8443", RGOE_GATEWAY_REGION: "eu" };
  const calls = [];
  let fail = false;
  ...
  ok(scheduled && scheduled.ms === 7000, "re-announce scheduled at exactly RGOE_BOOTNODE_HEARTBEAT*1000 ms");
  const a0 = calls[0];
  ...
  ok(a0.op.operator && a0.op.operatorSig && await verifyOperatorSig(gw.onion, a0.op.operator, a0.op.operatorSig), "operator resolved from RGOE_GW_OPERATOR_KEY and its sig verifies for THIS onion");
  ...
  const rlogs2 = [];
  const h2 = await runHeartbeat({ env: { RGOE_BOOTNODE_ONION: V.onion, RGOE_EGRESS_CHECK: "0" }, log: (s) => rlogs2.push(s), schedule: fakeSchedule, announce: fakeAnnounce, egress: async () => { throw new Error("must not probe"); }, identity: gw });
  ...
  ok((await throwsWith(() => runHeartbeat({ env: {}, log: () => {}, schedule: fakeSchedule, announce: fakeAnnounce, identity: gw }), /set RGOE_BOOTNODE_ONION/)).threw, "runHeartbeat without a bootnode fails fast");
  ok((await throwsWith(() => runHeartbeat({ env: { RGOE_BOOTNODE_ONION: V.onion, RGOE_GW_OPERATOR: OA.operator }, log: () => {}, schedule: fakeSchedule, announce: fakeAnnounce, identity: gw }), /must be set together/)).threw, "runHeartbeat with half an operator pair fails at startup (before any announce)");
  ...
  const badEnvs = [
    { RGOE_GW_OPERATOR_KEY: opKey + "5" }, { RGOE_GW_OPERATOR_KEY: opKey.slice(0, -1) }, { RGOE_GW_OPERATOR_KEY: "0x" + "ff".repeat(32) },
    { RGOE_GW_OPERATOR: OA.operator }, { RGOE_GW_OPERATOR: OA.operator, RGOE_GW_OPERATOR_SIG: flipped },
  ];
  let errCaptured = "";
  for (const bad of badEnvs) {
    errCaptured += await withCapture(async () => {
      try { await runHeartbeat({ env: { RGOE_BOOTNODE_ONION: V.onion, ...bad }, schedule: fakeSchedule, announce: fakeAnnounce, identity: gw }); }
      catch (e) { console.error(e); } // exactly what main() does
    });
  }
  ok(errCaptured.split("RGOE_GW_OPERATOR").length >= badEnvs.length, "control: every misconfig printed an error via console.error(e)");
