...
//
//   shade-tree join member   -> self-enrolls: prints a well-formed commitment + the EXACT client
//                         command on stdout, and the bearer secret on STDERR only (never stdout,
//                         mirroring enroll's commitment-only stream contract).
//   shade-tree join gateway  -> mints a real .onion identity into a temp dir and prints the gateway +
...
async function main() {
  ...
  ok(/shade-tree client --secret \$SHADE_TREE_SECRET/.test(m.stdout), "prints the `shade-tree client --secret $SHADE_TREE_SECRET ...` command");
  ...
  try {
    ...
    const g = shadeTree(["join", "gateway", hsDir], { timeout: 40_000 });
    ok(g.code === 0, "`shade-tree join gateway <hsDir>` exits 0");
    ...
    ok(/shade-tree gateway/.test(g.stdout), "prints the `shade-tree gateway` command");
    ok(/shade-tree heartbeat --bootnode <bootnode-onion>/.test(g.stdout), "prints the `shade-tree heartbeat --bootnode <onion>` command");
