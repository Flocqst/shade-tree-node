// Selftest for `rgoe identity` (group/identity.mjs + lib/identity-file.mjs): the Rust client's
...
//     leaf `rgoe enroll` publishes / the on-chain slash names — and `identitySecret` ==
...
//     rust/rgoe-rln/interop/egress-derive.mjs writes (byte-for-byte), so the harness and the
//     command cannot drift;
//   - secret sourcing: --secret-file > RGOE_SECRET (router --secret) > ./.secret; no secret => exit 1;
...
const DERIVE = join(ROOT, "rust", "rgoe-rln", "interop", "egress-derive.mjs");

// A fixed test secret (0x-hex, as `rgoe enroll` mints). Never a real member.
...
async function main() {
  const work = await mkdtemp(join(tmpdir(), "rgoe-identity-"));
  try {
    ...
    // ---- stdout mode: JSON alone on stdout, secret sourced from RGOE_SECRET -----------------
    console.log("\nrgoe identity (RGOE_SECRET, stdout):");
    const so = run([], { env: { RGOE_SECRET: SECRET } });
    ...
    ok(/RGOE_SECRET/.test(so.stderr) && so.stderr.includes(refLeaf), "stderr names the source and the PUBLIC leaf");
    ...
    // ---- router flag: --secret sets RGOE_SECRET ---------------------------------------------
    console.log("\nrgoe identity --secret (router flag -> RGOE_SECRET):");
    const sf = run(["--secret", SECRET]);
    ok(sf.code === 0 && sf.stdout === so.stdout, "--secret yields the same file as RGOE_SECRET");
    ...
    const wo = run(["--out", outPath], { env: { RGOE_SECRET: SECRET } });
    ...
    ok(Buffer.compare(ours, theirs) === 0, "rgoe identity --out == egress-derive.mjs identity.json BYTE-FOR-BYTE");
    ...
    const lo = run(["--out", loose], { env: { RGOE_SECRET: SECRET } });
    ...
    const fs1 = run(["--secret-file", secretFile], { env: { RGOE_SECRET: OTHER } });
    ok(fs1.code === 0 && fs1.stdout === so.stdout, "--secret-file wins over RGOE_SECRET (trailing newline trimmed)");
    ...
    const badSecret = run([], { env: { RGOE_SECRET: "not-a-number" } });
    ...
    // ---- arg validation ---------------------------------------------------------------------
    console.log("\nargument validation:");
    ok(run(["--bogus", "x"], { env: { RGOE_SECRET: SECRET } }).code === 2, "unknown flag => exit 2");
    ok(run(["--out"], { env: { RGOE_SECRET: SECRET } }).code === 2, "--out without a value => exit 2");
    ok(run(["stray"], { env: { RGOE_SECRET: SECRET } }).code === 2, "stray positional => exit 2");
    const help = run(["--help"]);
  ...
  console.log(`\n${failures === 0 ? "PASS" : "FAIL"}: rgoe identity selftest (${failures} failure${failures === 1 ? "" : "s"})`);
