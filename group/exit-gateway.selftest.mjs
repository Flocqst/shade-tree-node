// Selftest for `rgoe exit-gateway` / `rgoe withdraw-gateway` / `rgoe gateway-status`
...
//   `rgoe register-gateway`, status, withdraw-before-exit refused (NotExiting), exit --dry-run
...
function rgoe(args, env = {}, timeout = 60_000) {
...
async function part1() {
  ...
  const OFF = { RGOE_GATEWAY_REGISTRY: REG, RGOE_RPC_URL: "http://127.0.0.1:1" };
  const ex = rgoe(["exit-gateway", "--dry-run"], OFF);
  ...
  const wd = rgoe(["withdraw-gateway", "--dry-run", "--recipient", RCPT], OFF);
  ...
  const wdDefault = rgoe(["withdraw-gateway", "--dry-run"], OFF);
  ...
  const nokey = rgoe(["exit-gateway", "--dry-run", "--operator", RCPT], { ...OFF, RGOE_RPC_URL: "http://10.255.255.1:1" });
  ...
  const send = rgoe(["exit-gateway"], { ...OFF, RGOE_RPC_URL: "http://10.255.255.1:1" }, 20_000);
  ok(send.code === 1 && !/DRY-RUN/.test(send.out), "a real send with the RPC unreachable exits 1 (never pretends)");

  console.log("\nvalidation:");
  ok(rgoe(["exit-gateway", "--bogus"], OFF).code === 2, "unknown flag => exit 2");
  ok(rgoe(["exit-gateway", "--recipient", RCPT], OFF).code === 2, "--recipient on exit => exit 2 (withdraw only)");
  ok(rgoe(["exit-gateway", "--dry-run", "--operator", "0xnope"], OFF).code === 1, "malformed --operator => exit 1");
  ok(rgoe(["withdraw-gateway", "--dry-run", "--recipient", "12345"], OFF).code === 1, "malformed --recipient => exit 1");
  ok(rgoe(["exit-gateway", "--dry-run"], { RGOE_RPC_URL: "http://127.0.0.1:1" }).code === 1, "no registry address anywhere => exit 1");
  ok(rgoe(["exit-gateway", "--dry-run", "--operator", RCPT], { ...OFF, RGOE_REGISTER_KEY: "0xdead" }).code === 1, "malformed RGOE_REGISTER_KEY => exit 1");
  const mism = rgoe(["exit-gateway", "--dry-run", "--operator", RCPT], { ...OFF, RGOE_REGISTER_KEY: ANVIL_KEY_1 });
  ...
  const help = rgoe(["withdraw-gateway", "--help"]);
  ok(help.code === 0 && /withdraw/.test(help.out), "`rgoe withdraw-gateway --help` exits 0");
  const top = rgoe(["help"]);
  ok(/exit-gateway/.test(top.out) && /withdraw-gateway/.test(top.out) && /gateway-status/.test(top.out) && /\bidentity\b/.test(top.out), "`rgoe help` lists exit-gateway, withdraw-gateway, gateway-status, identity");
}
...
async function part2() {
  ...
  const work = await mkdtemp(join(tmpdir(), "rgoe-exitgw-"));
  try {
    let up = false;
    ...
    const ENV = { RGOE_RPC_URL: url, RGOE_GATEWAY_REGISTRY: reg };
    ...
    const r = rgoe(["register-gateway"], ENV);
    ok(r.code === 0 && (await isStaked(ANVIL_ADDR_1)) === true, "`rgoe register-gateway` stakes anvil #1 (fixture)");
    const s0 = rgoe(["gateway-status"], ENV);
    ok(s0.code === 0 && /state:\s+staked \(active\)/.test(s0.out) && s0.out.includes(`BOND:     ${BOND}`) && s0.out.includes(`UNBONDING: ${UNBONDING}`), "`rgoe gateway-status` reads staked (active) + BOND/UNBONDING");
    ok(!s0.out.includes(KEY_1_HEX), "status never prints the key");
    const sOther = rgoe(["gateway-status", "--operator", RCPT], { ...ENV, RGOE_REGISTER_KEY: "" });
    ...
    const w0 = rgoe(["withdraw-gateway"], ENV);
    ...
    const ed = rgoe(["exit-gateway", "--dry-run"], ENV);
    ...
    const e1 = rgoe(["exit-gateway"], ENV);
    const st1 = await stakes(ANVIL_ADDR_1);
    ok(e1.code === 0 && /mined in block/.test(e1.out) && /withdrawable at/.test(e1.out), "`rgoe exit-gateway` mines initiateExit and reports withdrawableAt");
    ...
    // exit again: no-op
    const e2 = rgoe(["exit-gateway"], ENV);
    ...
    const w1 = rgoe(["withdraw-gateway"], ENV);
    ...
    const s1 = rgoe(["gateway-status"], ENV);
    ...
    const wd = rgoe(["withdraw-gateway", "--dry-run", "--recipient", RCPT], ENV);
    ...
    const w2 = rgoe(["withdraw-gateway", "--recipient", RCPT, "--key-file", keyFile], { ...ENV, RGOE_REGISTER_KEY: "" });
    ok(w2.code === 0 && /paid to/.test(w2.out) && /--key-file/.test(w2.out), "`rgoe withdraw-gateway --key-file` mines withdraw(recipient)");
    ...
    const w3 = rgoe(["withdraw-gateway"], ENV);
    ok(w3.code === 1 && /NotStaked/.test(w3.out), "withdraw after withdraw is refused (NotStaked)");
    const e3 = rgoe(["exit-gateway"], ENV);
    ok(e3.code === 1 && /NotStaked/.test(e3.out), "exit when unstaked is refused (NotStaked)");

    // keystore signer path (Foundry-style encrypted JSON) with RGOE_KEYSTORE_PASSWORD; light scrypt for test speed
    ...
    const ks = rgoe(["gateway-status", "--keystore", ksPath], { ...ENV, RGOE_KEYSTORE_PASSWORD: "hunter2", RGOE_REGISTER_KEY: "" });
    ok(ks.code === 0 && ks.out.includes(ANVIL_ADDR_1) && /--keystore/.test(ks.out), "--keystore decrypts with RGOE_KEYSTORE_PASSWORD and derives the operator address");
    const ksBad = rgoe(["gateway-status", "--keystore", ksPath], { ...ENV, RGOE_KEYSTORE_PASSWORD: "wrong", RGOE_REGISTER_KEY: "" });
    ok(ksBad.code === 1 && /could not decrypt/.test(ksBad.out), "wrong keystore password => exit 1");
    const acct = rgoe(["gateway-status", "--account", "op"], { ...ENV, RGOE_KEYSTORE_PASSWORD: "hunter2", FOUNDRY_KEYSTORES: work, RGOE_REGISTER_KEY: "" });
    ...
    const acct2 = rgoe(["gateway-status", "--account", "op"], { ...ENV, RGOE_KEYSTORE_PASSWORD: "hunter2", FOUNDRY_KEYSTORES: work, RGOE_REGISTER_KEY: "" });
...
async function main() {
  await part1();
  await part2();
  console.log(`\n${failures === 0 ? "PASS" : "FAIL"}: rgoe exit-gateway selftest (${failures} failure${failures === 1 ? "" : "s"})`);
