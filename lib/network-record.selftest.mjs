...
function main() {
  ...
  console.log("\ncommitted network/sepolia records:");
  {
    ...
    ok(d.SHADE_TREE_RPC_URL === r.contracts.rpcUrl, "sepolia rpcUrl resolves to SHADE_TREE_RPC_URL");
    ok(d.SHADE_TREE_GROUP_CONTRACT === r.contracts.contracts.stakedReputationSet, "sepolia stakedReputationSet -> SHADE_TREE_GROUP_CONTRACT");
    ok(d.SHADE_TREE_DIR_SIGNER && d.SHADE_TREE_DIR_SIGNER.length >= 64, "sepolia resolves a pinned signer (live bootnode or static directory)");
    ok(Boolean(d.SHADE_TREE_BOOTNODE_ONION) || Boolean(d.SHADE_TREE_DIRECTORY), "sepolia resolves a discovery source (bootnode onion or static SHADE_TREE_DIRECTORY)");
    if (r.contracts.contracts.gatewayRegistry === null) ok(!("SHADE_TREE_GATEWAY_REGISTRY" in d), "null gatewayRegistry supplies NO SHADE_TREE_GATEWAY_REGISTRY default");
  ...
  console.log("\nenv resolution (temp network dir):");
  {
    ...
    ok(!("SHADE_TREE_BOOTNODE_ONION" in rt) && !("SHADE_TREE_DIR_SIGNER" in rt), "retired bootnode supplies no discovery defaults");
