...
function main() {
  ...
  console.log("\ncommitted network/sepolia records:");
  {
    ...
    ok(Object.keys(d).length === 0, "retired Sepolia records supply no v4 runtime defaults");
    let retiredRejected = false;
    try { networkEnvDefaults("sepolia"); } catch (e) { retiredRejected = /retired deployment record.*unset SHADE_TREE_NETWORK/.test(e.message); }
    ok(retiredRejected, "selecting retired Sepolia by name fails clearly instead of falling through to local discovery");
