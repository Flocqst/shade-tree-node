...
function main() {
  ...
  {
    const { logs } = withEnv({ SHADE_TREE_LOG_FORMAT: undefined, SHADE_TREE_LOG_LEVEL: undefined }, () =>
  ...
  }
  {
    ...
    const { logs } = withEnv({ SHADE_TREE_LOG_FORMAT: "text" }, () => capture(() => log.info("gateway up on 127.0.0.1:8443")));
  ...
  }
  {
    ...
    const { logs } = withEnv({ SHADE_TREE_LOG_FORMAT: "text" }, () => capture(() => log.info("m", { a: "two words", b: "" })));
  ...
  console.log("\njson mode:");
  {
    ...
    const { logs } = withEnv({ SHADE_TREE_LOG_FORMAT: "json" }, () =>
  ...
  }
  {
    ...
    const { warns } = withEnv({ SHADE_TREE_LOG_FORMAT: "json" }, () => capture(() => log.warn("drop", { reason: "bad-target" })));
  ...
  console.log("\nlevel filtering:");
  {
    const { logs } = withEnv({ SHADE_TREE_LOG_LEVEL: undefined }, () => capture(() => log.debug("noise", { x: 1 })));
    ok(logs.length === 0, "debug suppressed at default (info) level");
  }
  {
    const { logs } = withEnv({ SHADE_TREE_LOG_LEVEL: "debug" }, () => capture(() => log.debug("noise")));
    ok(logs.length === 1, "debug emitted when SHADE_TREE_LOG_LEVEL=debug");
  }
  {
    const { logs, warns, errors } = withEnv({ SHADE_TREE_LOG_LEVEL: "error" }, () =>
  ...
  console.log("\nsink routing:");
  {
    const { logs, warns, errors } = withEnv({ SHADE_TREE_LOG_FORMAT: "text", SHADE_TREE_LOG_LEVEL: "debug" }, () =>
  ...
  {
    const { logs } = withEnv({ SHADE_TREE_LOG_FORMAT: "json" }, () => capture(() => log.info("m", { epoch: 7n })));
  ...
  }
  {
    ...
    withEnv({ SHADE_TREE_LOG_FORMAT: "text" }, () => { logs = capture(() => { try { log.info("m", { o: circ }); } catch { threw = true; } }).logs; });
