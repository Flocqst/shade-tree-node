...
//   2. SIGNED UNDER THE ONION KEY (bootnode can't forge). Caps ride inside the main
...
//      can be re-verified against the entry's own onion key WITHOUT trusting the bootnode
//      or the directory signer. Altering any real cap field breaks a signature -> rejected.
...
//      envelope {min,max} range. `canonicalCaps` is TOTAL: any garbage/unknown field is
//      dropped, never thrown on, so a malformed caps is ignored safely rather than fatal.
export const CAPS_DOMAIN = "RGOE gateway capabilities v1\n";
...
export const DEFAULT_PROTO_VERSION = 3;

// Normalize a caps object into canonical, bucketed form with FIXED field order
// (ports, region, proto). TOTAL: never throws; unknown/invalid fields are dropped;
// returns {} when nothing valid remains (which every canonical builder treats as absent).
...
export function hasCaps(caps) {
  ...
  return c.ports !== undefined || c.region !== undefined || c.proto !== undefined;
}
...
function normalizePinnedSigners(pinned) {
  // Lowercase + strip an optional `0x` prefix so an operator who pastes `RGOE_DIR_SIGNER=0xabc..`
...
function checkGatewayBindings(dir) {
  for (const g of dir.gateways || []) {
    ...
    if (hasCaps(g.caps)) {
      if (!verifyCapsSig(g.onion, g.caps, g.capsSig)) {
...
export function verifyDirectoryThreshold(dir, pinnedSigner) {
...
export function verifyDirectory(dir, pinnedSigner) {
  ...
  if (isThresholdDirectory(dir)) return verifyDirectoryThreshold(dir, pinnedSigner);
