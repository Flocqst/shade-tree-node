...
//     RGOE_PAY_PROTOCOLS;
//   - the heartbeat producer: advertisedAdmits (RGOE_ADMIT / the RGOE_ROOTS alias / unset), advertisedPay
...
async function main() {
  ...
  console.log("\nsignatures + directory verification:");
  {
    ...
    ok(canonicalCapsBytes(g.onion, caps).toString("utf8").startsWith("RGOE gateway capabilities v1\n{\"onion\":"), "domain-separated, onion-bound bytes");
  ...
  const work = await mkdtemp(join(tmpdir(), "rgoe-admission-caps-"));
  try {
    ...
    // payAdvertFromEnv honours RGOE_PAY_PROTOCOLS
    const envBase = { RGOE_REGISTRAR_ADVERTISE: "1", RGOE_PAY_ASSET: ASSET, RGOE_PAY_PRICES: "8=100000,32=400000" };
    ok(J(payAdvertFromEnv(envBase).protocols) === '["x402","mpp"]' && J(payAdvertFromEnv({ ...envBase, RGOE_PAY_PROTOCOLS: "mpp" }).protocols) === '["mpp"]' && J(payAdvertFromEnv({ ...envBase, RGOE_PAY_PROTOCOLS: "MPP, x402" }).protocols) === '["x402","mpp"]', "payAdvertFromEnv: protocols = RGOE_PAY_PROTOCOLS (default both; canonical order)");
    ok(payAdvertFromEnv({ ...envBase, RGOE_PAY_PROTOCOLS: "lightning" }) === null, "an unknown rail -> no advert at all (never a half-truth in /health)");
  } finally {
  ...
  {
    ok(advertisedAdmits({}) === null, "RGOE_ADMIT unset (and no RGOE_ROOTS) -> not advertised (null)");
    ok(J(advertisedAdmits({ RGOE_ADMIT: "paid,invited" })) === '["invited","paid"]', "RGOE_ADMIT -> canonical list");
    ok(J(advertisedAdmits({ RGOE_ROOTS: "static,onchain", RGOE_GROUP_CONTRACT: "0xA" })) === '["invited","staked"]' && J(advertisedAdmits({ RGOE_ROOTS: "static,onchain", RGOE_GROUP_CONTRACT: "0xA", RGOE_PAID_ACCESS_CONTRACT: "0xP" })) === '["invited","staked","paid"]', "the deprecated RGOE_ROOTS alias maps over the contracts THIS env names");
    ok(J(advertisedAdmits({ RGOE_ADMIT: "invited", RGOE_ROOTS: "static,onchain", RGOE_GROUP_CONTRACT: "0xA" })) === '["invited"]', "RGOE_ADMIT wins over RGOE_ROOTS");
    let threw = null; try { advertisedAdmits({ RGOE_ADMIT: "onchain" }); } catch (e) { threw = e.message; }
    ok(/unknown admission path "onchain"/.test(threw || ""), "a malformed RGOE_ADMIT is a startup error in the heartbeat too (never advertise a guess)");
    const gwOnion = newOnion().onion, bnOnion = newOnion().onion;
    const envPay = { RGOE_REGISTRAR_ADVERTISE: "1", RGOE_PAY_ASSET: ASSET, RGOE_PAY_PRICES: "8=100000,32=400000", RGOE_PAY_PROTOCOLS: "x402" };
    ok(advertisedPay({}) === null && advertisedPay({ ...envPay, RGOE_REGISTRAR_ADVERTISE: "0" }) === null, "no RGOE_REGISTRAR_ADVERTISE -> no pay");
    const p1 = advertisedPay(envPay, { gatewayOnion: gwOnion });
    ok(p1 && J(p1.protocols) === '["x402"]' && p1.port === 8878 && p1.asset === ASSET && p1.chain === "eip155:11155111" && J(p1.tiers) === '{"8":"100000","32":"400000"}' && p1.onion === undefined, "advertisedPay: the /health advert shape; no onion when the registrar rides the gateway's own onion (RGOE_REGISTRAR_ONION unset)");
    ok(advertisedPay({ ...envPay, RGOE_REGISTRAR_ONION: gwOnion.toUpperCase() }, { gatewayOnion: gwOnion }).onion === undefined, "RGOE_REGISTRAR_ONION == the gateway onion -> no onion field");
    ok(advertisedPay({ ...envPay, RGOE_REGISTRAR_ONION: bnOnion.replace(/\.onion$/, "") }, { gatewayOnion: gwOnion }).onion === bnOnion, "RGOE_REGISTRAR_ONION = the BOOTNODE onion -> pay.onion names it (with .onion normalized)");
    ok(buildGatewayCaps({}) === null && buildGatewayCaps({ RGOE_UNRELATED: "1" }) === null, "buildGatewayCaps: unconfigured -> null (byte-identical legacy announce)");
    const c1 = buildGatewayCaps({ RGOE_ADMIT: "invited,staked,paid", ...envPay, RGOE_REGISTRAR_ONION: bnOnion }, { gatewayOnion: gwOnion });
    ok(c1 && J(c1.admits) === '["invited","staked","paid"]' && c1.pay && c1.pay.onion === bnOnion && c1.proto && c1.ports === undefined, "RGOE_ADMIT + advert -> caps { admits, pay, proto } (proto rides along; no ports without an egress policy)");
    const c2 = buildGatewayCaps({ RGOE_ADMIT: "invited" });
    ok(c2 && J(c2.admits) === '["invited"]' && c2.pay === undefined && c2.proto, "RGOE_ADMIT=invited alone -> caps { admits:[invited], proto } (an invited-only gateway IS advertised, so --max-anon clients can find it)");
