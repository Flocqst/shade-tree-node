...
// -- bootnode/deploy/bootstrap.sh RGOE_HELIOS=1), whose `eth_getBlockByNumber` only returns
// headers that chain to a beacon-chain sync-committee-verified execution payload.
//
// Trust, stated plainly: with this hook wired the RPC (`RGOE_RPC_URL`) is a dumb data pipe.
...
// the sidecar binds 127.0.0.1 and the unit is sandboxed like the other rgoe units.
//
...
// it. Expected chain: `chainId` option, else RGOE_HELIOS_CHAIN_ID, else the upstream RPC's own
...
//   "finalized" -> eth_getBlockByNumber("finalized")   the recommended setting (RGOE_CONFIRMATIONS=0)
...
//   rpcUrl          Helios local JSON-RPC (RGOE_HELIOS_RPC_URL), e.g. http://127.0.0.1:8545
//   chainId         expected chain (number|bigint|hex string); default RGOE_HELIOS_CHAIN_ID
//   upstreamRpcUrl  the ordinary RPC (RGOE_RPC_URL): used to derive the expected chain when
...
export function makeHeliosTrustedStateRoot({
  rpcUrl = process.env.RGOE_HELIOS_RPC_URL,
  chainId = process.env.RGOE_HELIOS_CHAIN_ID,
  upstreamRpcUrl = process.env.RGOE_RPC_URL || "http://127.0.0.1:8545",
  timeoutMs = 10_000,
} = {}) {
  if (!rpcUrl) throw new Error("makeHeliosTrustedStateRoot needs rpcUrl (RGOE_HELIOS_RPC_URL)");
  let u;
  try { u = new URL(rpcUrl); } catch { throw new Error(`RGOE_HELIOS_RPC_URL is not a URL: ${rpcUrl}`); }
  if (u.protocol !== "http:" && u.protocol !== "https:") throw new Error(`RGOE_HELIOS_RPC_URL must be http(s): ${rpcUrl}`);
  const expectedGiven = chainId != null && chainId !== "" ? parseChainId(chainId, "configured RGOE_HELIOS_CHAIN_ID") : null;
