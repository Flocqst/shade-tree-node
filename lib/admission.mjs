...
//
// A gateway PROVIDER chooses which membership roots it honours (`RGOE_ADMIT`) and, if it sells
// access, which payment rails (`RGOE_PAY_PROTOCOLS`). These are the pure, dependency-free
...
//
// `RGOE_ROOTS` (T-FEAT-7: static,onchain) stays as a DEPRECATED alias: static -> invited,
...
// Parse an `RGOE_ADMIT` spec ("invited,paid", case/space tolerant) into the canonical
...
export function parseAdmit(spec, { name = "RGOE_ADMIT" } = {}) {
...
// The deprecated `RGOE_ROOTS` alias -> admission paths. `hasStaked` / `hasPaid` say which
...
export function admitsFromRoots(spec, { hasStaked = false, hasPaid = false } = {}) {
  const raw = String(spec ?? "").trim();
  const parts = raw.split(",").map((x) => x.trim().toLowerCase()).filter(Boolean);
  for (const p of parts) if (p !== "static" && p !== "onchain") throw new Error(`RGOE_ROOTS: unknown source "${p.slice(0, 16)}" (expected static,onchain; RGOE_ROOTS is deprecated -- use RGOE_ADMIT=invited,staked,paid)`);
  const st = parts.includes("static"), on = parts.includes("onchain");
  if (!st && !on) throw new Error("RGOE_ROOTS: no source selected (RGOE_ROOTS is deprecated -- use RGOE_ADMIT=invited,staked,paid)");
  if (on && !hasStaked && !hasPaid) throw new Error("RGOE_ROOTS names onchain but no contract is configured (set RGOE_GROUP_CONTRACT and/or RGOE_PAID_ACCESS_CONTRACT); RGOE_ROOTS is deprecated -- use RGOE_ADMIT");
  const out = [];
...
// Parse `RGOE_PAY_PROTOCOLS` ("x402,mpp"; any non-empty subset; unset/empty = both) into the
// canonical ordered list. Throws on an unknown rail.
export function parsePayProtocols(spec, { name = "RGOE_PAY_PROTOCOLS" } = {}) {
  const raw = String(spec ?? "").trim();
...
// The client's leaf-source pin: `RGOE_LEAF_SOURCE=auto|invited|staked|paid` (default auto).
export function parseLeafSource(spec, { name = "RGOE_LEAF_SOURCE" } = {}) {
...
// Boolean env/flag: `1`, `true`, `yes`, `on` => true (RGOE_MAX_ANON; `--max-anon` maps to "true").
