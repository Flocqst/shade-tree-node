...
// One epoch = one rate-limit window AND the nullifier-rotation period.
// Default 86400s (a day), so a whole demo session sits inside ONE epoch: proofs
// don't regenerate mid-session and friends never hit an epoch boundary. The
// tradeoff is a longer linkable window (same member => same nullifier all day);
// for a chill, prove-it-works demo that is an intentional, fine choice. Shorten
// it (e.g. 3600) for tighter unlinkability.
//
// IMPORTANT: this value must be IDENTICAL on the client and the gateway, or every
// proof lands in the wrong epoch and is dropped. Both run this same file, so the
// default already matches everywhere; only override RGOE_EPOCH_SECONDS if you set
// it on BOTH sides.
export const EPOCH_SECONDS = Number(process.env.RGOE_EPOCH_SECONDS || 86400);
