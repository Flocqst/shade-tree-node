...
//                            at `finalized` (or head - RGOE_CONFIRMATIONS). Trusts the RPC the
...
//   - MockStakeVerifier    : chainless dev. RGOE_STAKE_ALLOWLIST=addr,addr (staked set), or
...
// Registry address + RPC: explicit env wins; else the RGOE_NETWORK record (network/<name>/
// contracts.json `contracts.gatewayRegistry` / `rpcUrl`, lib/network-record.mjs); else dev defaults.
// networkDefault() never throws (null when RGOE_NETWORK is unset or the slot is null = undeployed).
const registryAddress = () => process.env.RGOE_GATEWAY_REGISTRY || networkDefault("RGOE_GATEWAY_REGISTRY");
const RPC_URL = process.env.RGOE_RPC_URL || networkDefault("RGOE_RPC_URL") || "http://127.0.0.1:8545";
// Stake is an admission/liveness read, not a slash trigger, so it defaults to `latest`
// (dev chains like anvil don't advance `finalized` promptly). Set RGOE_CONFIRMATIONS>0
// to read at head-N for reorg safety on a public chain.
const CONFIRMATIONS = Number(process.env.RGOE_CONFIRMATIONS || 0); // 0 => latest
const CACHE_MS = Number(process.env.RGOE_STAKE_CACHE_MS || 15000);
...
export function OnchainStakeVerifier({ rpcUrl = RPC_URL, contract = registryAddress(), cacheMs = CACHE_MS } = {}) {
  if (!contract) throw new Error("OnchainStakeVerifier needs RGOE_GATEWAY_REGISTRY (GatewayRegistry address; or RGOE_NETWORK with contracts.gatewayRegistry deployed)");
...
export function MockStakeVerifier({ allowlist = process.env.RGOE_STAKE_ALLOWLIST } = {}) {
  const set = allowlist
...
export function makeStakeVerifier(mode = process.env.RGOE_STAKE_MODE) {
  ...
  throw new Error(`unknown RGOE_STAKE_MODE: ${resolved} (expected onchain|mock)`);
}
