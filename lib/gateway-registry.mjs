...
export function OnchainStakeVerifier({ rpcUrl = RPC_URL, contract = process.env.RGOE_GATEWAY_REGISTRY, cacheMs = CACHE_MS } = {}) {
  if (!contract) throw new Error("OnchainStakeVerifier needs RGOE_GATEWAY_REGISTRY (GatewayRegistry address)");
...
export function makeStakeVerifier(mode = process.env.RGOE_STAKE_MODE) {
  const resolved = mode || (process.env.RGOE_GATEWAY_REGISTRY ? "onchain" : "mock");
