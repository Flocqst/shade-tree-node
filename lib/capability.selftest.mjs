...
async function main() {
  ...
  // Env must be set BEFORE importing client/selection.mjs (it reads RGOE_DIRECTORY /
  // RGOE_DIR_SIGNER at import), so we build a signed directory file first, then dynamic-import.
  const work = await mkdtemp(join(tmpdir(), "rgoe-caps-"));
  try {
    const signer = newSigner();
    const full = newOnion();    // ports 80+443, region eu, proto 3-4
    const https = newOnion();   // ports 443 only, region na, proto 3-3
    const legacy = newOnion();  // NO caps (legacy gateway)
    const dir = {
      version: 1,
      issued: Math.floor(Date.now() / 1000),
      gateways: [
        capEntry(full, { ports: [80, 443], region: "eu", proto: { min: 3, max: 4 } }),
    ...
    ok(DEFAULT_EGRESS_PORT === 443 && DEFAULT_PROTO_VERSION === 3, "defaults are the documented 443 / proto-3 floor");
    ...
    ok(sel.gatewayMeetsRequirement(fleet[2], { proto: 3 }) === true && sel.gatewayMeetsRequirement(fleet[2], { proto: 4 }) === false,
       "a no-caps gateway is assumed proto {3} only");
    ...
    ok(pr4.length === 1 && onionsOf(pr4).has(shortFull), "selectCandidates({proto:4}) selects ONLY the gateway whose range covers v4");
    ...
    ok(conj.length === 1 && onionsOf(conj).has(shortFull), "a combined requirement is a conjunction (port AND proto)");
