...
//     Plus the start-block resolution: RGOE_FROM_BLOCKS > RGOE_FROM_BLOCK > network record > 0.
...
async function pagedLogs() {
  ...
  {
    const rpc = await fakeRpc({ logs, cap: 50_000, head: 101_000 });
    const saved = { RGOE_FROM_BLOCK: process.env.RGOE_FROM_BLOCK, RGOE_FROM_BLOCKS: process.env.RGOE_FROM_BLOCKS, RGOE_LOGS_CHUNK: process.env.RGOE_LOGS_CHUNK, RGOE_CONFIRMATIONS: process.env.RGOE_CONFIRMATIONS };
    process.env.RGOE_FROM_BLOCK = "0x0"; process.env.RGOE_LOGS_CHUNK = "10000"; delete process.env.RGOE_FROM_BLOCKS; delete process.env.RGOE_CONFIRMATIONS;
    try {
  ...
  // i. start-block resolution: RGOE_FROM_BLOCKS (per contract) > RGOE_FROM_BLOCK > the network
  //    record's deploy block (RGOE_NETWORK, else any record naming the address) > 0x0; and
  //    applyNetworkEnv fills BOTH from the record unless either is pinned.
  {
    ...
    ok(fromBlockFor(PAID, { RGOE_FROM_BLOCK: "0x5" }) === "0x5", "RGOE_FROM_BLOCK beats the record");
    ok(fromBlockFor(PAID, { RGOE_FROM_BLOCK: "0x5", RGOE_FROM_BLOCKS: `${PAID.toLowerCase()}=7,${STAKED}=0x9` }) === "0x7" && fromBlockFor(STAKED, { RGOE_FROM_BLOCK: "0x5", RGOE_FROM_BLOCKS: `${PAID}=7,${STAKED}=0x9` }) === "0x9" && fromBlockFor(UNKNOWN, { RGOE_FROM_BLOCK: "0x5", RGOE_FROM_BLOCKS: `${PAID}=7` }) === "0x5", "RGOE_FROM_BLOCKS beats RGOE_FROM_BLOCK for the named contract (case-insensitive; decimal or hex); others fall through");
    let threw = null;
    try { fromBlockFor(PAID, { RGOE_FROM_BLOCKS: "0x1234=5" }); } catch (e) { threw = e.message; }
    ok(/RGOE_FROM_BLOCKS: bad entry/.test(threw || ""), "a malformed RGOE_FROM_BLOCKS entry throws (never silently scans from 0)");
    ok(parseFromBlocks("").size === 0 && parseFromBlocks(undefined).size === 0, "empty map ok");
    ok(deployBlockForContract(PAID, { env: {} }) === 11510873 && deployBlockForContract("nope", { env: {} }) === null, "deployBlockForContract: scans all records without RGOE_NETWORK; garbage -> null");
    // a temp record: RGOE_NETWORK resolution + applyNetworkEnv MIN + map, explicit env pinned
    const root = mkdtempSync(join(tmpdir(), "rgoe-frombk-"));
    try {
      ...
      const env = { RGOE_NETWORK: "testnet" };
      const filled = applyNetworkEnv(env, { root });
      ok(filled.includes("RGOE_FROM_BLOCK") && env.RGOE_FROM_BLOCK === "0x32" && env.RGOE_FROM_BLOCKS === `${A}=100,${B}=300,${G}=50`, `applyNetworkEnv fills RGOE_FROM_BLOCK = MIN deploy block (0x32 = 50) + the per-contract map (${env.RGOE_FROM_BLOCKS})`);
      ...
      const pinned = { RGOE_NETWORK: "testnet", RGOE_FROM_BLOCK: "0x0" };
      applyNetworkEnv(pinned, { root });
      ok(pinned.RGOE_FROM_BLOCK === "0x0" && !("RGOE_FROM_BLOCKS" in pinned) && fromBlockFor(B, pinned) === "0x0", "an explicit RGOE_FROM_BLOCK pins BOTH (no derived map out-ranks it): explicit env always wins");
      const pinnedMap = { RGOE_NETWORK: "testnet", RGOE_FROM_BLOCKS: `${B}=7` };
      applyNetworkEnv(pinnedMap, { root });
      ok(!("RGOE_FROM_BLOCK" in pinnedMap) && fromBlockFor(B, pinnedMap) === "0x7" && deployBlockForContract(A, { env: pinnedMap, root }) === 100, "an explicit RGOE_FROM_BLOCKS pins the map (RGOE_FROM_BLOCK not derived); an unnamed contract still resolves its record deploy block");
