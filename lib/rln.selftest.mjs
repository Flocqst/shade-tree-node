...
async function main() {
  ...
  const targetSwap = await verifyEnvelope({ ...env, target: "evil.example:443" }, recentRoots);
  ok("rejects a swapped target (proof not bound to it)", targetSwap.ok === false && targetSwap.reason === "target-not-bound");
  const nonceSwap = await verifyEnvelope({ ...env, nonce: "different" }, recentRoots);
  ok("rejects a swapped nonce", nonceSwap.ok === false && nonceSwap.reason === "target-not-bound");
  const noNonce = await verifyEnvelope({ ...env, nonce: undefined }, recentRoots);
  ...
  const bigNonce = await verifyEnvelope({ ...env, nonce: "z".repeat(500) }, recentRoots);
