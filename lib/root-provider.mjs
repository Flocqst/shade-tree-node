...
const FRESHNESS_ROOTS = () => Number(process.env.SHADE_TREE_FRESHNESS_ROOTS || 2); // current + how many prior
...
export const _internals = { verifyMptProof, decodeCompactPath, toNibbles, TOPIC, fetchLogsChunked, asBlockNumber, LOGS_CHUNK_FLOOR, pollOnChange, pollSnapshotKey };
...
function pollOnChange(currentRoots) {
  return function onChange(cb, intervalMs = 12000) {
    ...
    const run = async () => {
      try {
        const snapshot = await currentRoots();
        const next = pollSnapshotKey(snapshot);
        if (next !== last) {
          last = next;
          await cb(snapshot.roots || []);
        }
