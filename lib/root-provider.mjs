...
export function reconstructRoot(logs) {
  ...
  // MUST emit the rateCommitment as the indexed topic[1]. Removed-leaf handling here
  // rebuilds a fresh tree of survivors (renumbering indices); an RLN slash that zeroes a
  // leaf in place would diverge — fine for the append-only demo, a TODO for removals.
  return newGroup(active).root.toString();
}
