...
const LOGS_CHUNK = () => Math.max(1, Math.floor(Number(process.env.RGOE_LOGS_CHUNK) || 10_000));
...
//   1. RGOE_FROM_BLOCKS=<addr>=<block>,...   per-contract (each set scanned from its own deploy block)
//   2. RGOE_FROM_BLOCK=<block>               one start block for every contract
//   3. the contract's deploy block from a committed network record (network/<RGOE_NETWORK>/
//      contracts.json, else any record naming the address — lib/network-record.mjs
//      deployBlockForContract; RGOE_NETWORK's applyNetworkEnv also fills 1+2 from the record)
//   4. 0x0 (anvil / an unknown contract; chunked scanning below keeps even this correct, only slow)
// Explicit env always wins over the record. Exported for the selftest + `rgoe leaves`.
export function fromBlockFor(contract, env = process.env) {
  const map = parseFromBlocks(env.RGOE_FROM_BLOCKS);
  const per = contract ? map.get(String(contract).toLowerCase()) : undefined;
  if (per !== undefined) return "0x" + per.toString(16);
  if (env.RGOE_FROM_BLOCK) return env.RGOE_FROM_BLOCK;
...
export function NodeRootProvider({ rpcUrl = RPC_URL(), contract } = {}) {
  ...
  // (RGOE_CONFIRMATIONS>0) always rescan (a reorg deeper than N would otherwise stick).
...
//                    2026-08-17, docs/GO-LIVE-LOG-2026-08-17.md; Sepolia, cap 50k blocks)
...
// is not re-read per page), the range is split into RGOE_LOGS_CHUNK-block windows and a window the
...
// block (fromBlockFor: RGOE_FROM_BLOCKS / RGOE_FROM_BLOCK / the network record's deploy block).
...
export const _internals = { verifyMptProof, decodeCompactPath, toNibbles, TOPIC, fetchLogsChunked, asBlockNumber, LOGS_CHUNK_FLOOR };
