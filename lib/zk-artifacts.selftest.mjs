...
//     RGOE_ZK_ARTIFACTS `<id>=<path>` parsing; FAIL-CLOSED on a mislabeled id / missing file /
//     non-vkey / duplicate / empty; RGOE_ZK_ARTIFACT_LEGACY; legacyAccepted flips with the window
...
//   - heartbeat buildGatewayCaps advertises the accepted ids ONLY when RGOE_ZK_ARTIFACTS is set
...
import { buildEnvelope, RgoeClient } from "../client/rgoe-client.mjs";
...
const work = mkdtempSync(join(tmpdir(), "rgoe-zk-artifacts-"));
...
// ---- accepted set config -----------------------------------------------------------------
await test("default (RGOE_ZK_ARTIFACTS unset) == the built-in set under its own id, legacy accepted", () => {
...
await test("dual-VK window: RGOE_ZK_ARTIFACTS=<new>=..,<old>=.. accepts both; legacy = old by default (built-in)", () => {
  const set = loadArtifactSet({ env: { RGOE_ZK_ARTIFACTS: `${ALT}=${ALT_VKEY_PATH},${BUILTIN}=${BUILTIN_VKEY_PATH}` } });
  ...
  const set2 = loadArtifactSet({ env: { RGOE_ZK_ARTIFACTS: ALT_VKEY_PATH } });
  assert.deepEqual(set2.ids, [ALT]);
});

await test("window closed: only the new id accepted; RGOE_ZK_ARTIFACT_LEGACY names the retired id", () => {
  const set = loadArtifactSet({ env: { RGOE_ZK_ARTIFACTS: `${ALT}=${ALT_VKEY_PATH}`, RGOE_ZK_ARTIFACT_LEGACY: BUILTIN } });
  assert.deepEqual(set.ids, [ALT]);
  assert.equal(set.legacyId, BUILTIN);
  assert.equal(set.legacyAccepted, false, "outside the window the legacy id is RETIRED");
  assert.throws(() => loadArtifactSet({ env: { RGOE_ZK_ARTIFACT_LEGACY: "Bad Id" } }), /RGOE_ZK_ARTIFACT_LEGACY: bad artifact id/);
});

await test("FAIL CLOSED at load: mislabeled id, missing file, non-vkey JSON, duplicate id, empty spec", () => {
  assert.throws(() => loadArtifactSet({ env: { RGOE_ZK_ARTIFACTS: `${UNKNOWN}=${BUILTIN_VKEY_PATH}` } }), /artifact id mismatch .* declared rln-ffffffffffffffff but the file's content id is rln-/);
  assert.throws(() => loadArtifactSet({ env: { RGOE_ZK_ARTIFACTS: `${BUILTIN}=${join(work, "nope.json")}` } }), /vkey not found/);
  const notVk = join(work, "not-a-vkey.json");
  writeFileSync(notVk, JSON.stringify({ hello: 1 }));
  assert.throws(() => loadArtifactSet({ env: { RGOE_ZK_ARTIFACTS: notVk } }), /does not look like a snarkjs verification key/);
  writeFileSync(notVk, "{not json");
  assert.throws(() => loadArtifactSet({ env: { RGOE_ZK_ARTIFACTS: notVk } }), /not a JSON verification key/);
  assert.throws(() => loadArtifactSet({ env: { RGOE_ZK_ARTIFACTS: `${BUILTIN_VKEY_PATH},${BUILTIN_VKEY_PATH}` } }), /duplicate artifact id/);
  assert.throws(() => loadArtifactSet({ env: { RGOE_ZK_ARTIFACTS: " , " } }), /names no artifact/);
});

// ---- resolveArtifact ---------------------------------------------------------------------
const OPEN = loadArtifactSet({ env: { RGOE_ZK_ARTIFACTS: `${ALT}=${ALT_VKEY_PATH},${BUILTIN}=${BUILTIN_VKEY_PATH}` } });   // window open, legacy=BUILTIN
const CLOSED = loadArtifactSet({ env: { RGOE_ZK_ARTIFACTS: `${ALT}=${ALT_VKEY_PATH}`, RGOE_ZK_ARTIFACT_LEGACY: BUILTIN } }); // window closed
...
const nowMs = Number(epoch) * 120 * 1000 + 1000; // inside `epoch` regardless of RGOE_EPOCH_SECONDS default (120)
function consistentEnvelope(artifactField) {
  ...
  const env = { v: 3, target, nonce, proof: { snarkProof: { proof: {}, publicSignals: ps }, epoch: String(epoch), rlnIdentifier: "1" }, nullifier: "5", externalNullifier: ps.externalNullifier, share: { x, y: "7" } };
...
await test("gateway handler: artifact reject replies {ok:false, err:'gate:<reason>', artifacts:[accepted]} and counts the BOUNDED label", async () => {
  ...
  try {
    ...
    const before = metricValue("rgoe_gateway_requests_total", { result: "drop", reason: "artifact-retired" });
    ...
    assert.equal(metricValue("rgoe_gateway_requests_total", { result: "drop", reason: "artifact-retired" }), before + 1);
    assert.equal(metricValue("rgoe_gateway_requests_total", { result: "drop", reason: "artifact-unknown" }) >= 1, true);
    assert.equal(metricValue("rgoe_gateway_requests_total", { result: "drop", reason: "bad-artifact" }) >= 1, true);
...
await test("heartbeat buildGatewayCaps: advertises accepted ids only when RGOE_ZK_ARTIFACTS is set (default stays cap-free)", () => {
  assert.equal(buildGatewayCaps({}), null, "unconfigured => no caps (byte-identical announce)");
  const c = buildGatewayCaps({ RGOE_ZK_ARTIFACTS: `${ALT}=${ALT_VKEY_PATH},${BUILTIN}=${BUILTIN_VKEY_PATH}` });
  assert.deepEqual(c.artifacts, [ALT, BUILTIN].sort());
  assert.deepEqual(c.proto, { min: 3, max: 3 }, "proto rides along with a real cap");
  assert.equal(c.ports, undefined);
  assert.throws(() => buildGatewayCaps({ RGOE_ZK_ARTIFACTS: `${UNKNOWN}=${BUILTIN_VKEY_PATH}` }), /artifact id mismatch/, "a mis-pointed set aborts the heartbeat too (never advertise what we can't verify)");
...
await test("loadProverSets: default == the shipped set under the built-in id; RGOE_ZK_PROVER_ARTIFACTS is ordered newest-first + fail-closed", () => {
  ...
  const sets = loadProverSets({ env: { RGOE_ZK_PROVER_ARTIFACTS: `${ALT}=${altDir},${BUILTIN}=${RLN_DIR}` } });
  assert.deepEqual(sets.map((s) => s.id), [ALT, BUILTIN], "order preserved (newest first)");
  assert.throws(() => loadProverSets({ env: { RGOE_ZK_PROVER_ARTIFACTS: `${UNKNOWN}=${altDir}` } }), /artifact id mismatch/);
  assert.throws(() => loadProverSets({ env: { RGOE_ZK_PROVER_ARTIFACTS: join(work, "missing") } }), /missing/);
});
...
await test("RgoeClient._pickArtifact: intersects the first advertising candidate with its own ordered ids; no ad => newest; disjoint => precise reason", () => {
  const c = new RgoeClient({ secret: "0x01", onion: "a".repeat(56), artifacts: [ALT, BUILTIN] });
  ...
  const c2 = new RgoeClient({ secret: "0x01", onion: "a".repeat(56) });
...
await test("RgoeClient.connect: sends the negotiated `artifact`; an artifact-reject ack is surfaced precisely and its accepted list remembered", async () => {
  // accept
  const g1 = fakeGateway({ ok: true });
  const c1 = new RgoeClient({ secret: "0x01", onion: "a".repeat(56), artifacts: [ALT, BUILTIN], socksClient: g1.socksClient, dialAttempts: 1, prove: fakeProve, socksIsolation: false });
  c1.pool = fakePool;
  const tunnel = await c1.connect("h:443");
  assert.equal(tunnel.rgoe.artifact, ALT, "no ad => our newest");
  ...
  const c2 = new RgoeClient({ secret: "0x01", onion: "a".repeat(56), artifacts: [ALT, BUILTIN], gatewayArtifacts: [BUILTIN], socksClient: g2.socksClient, dialAttempts: 1, prove: fakeProve, socksIsolation: false });
  c2.pool = fakePool;
  assert.equal((await c2.connect("h:443")).rgoe.artifact, BUILTIN);
  ...
  const c3 = new RgoeClient({ secret: "0x01", onion: "a".repeat(56), artifacts: [ALT, BUILTIN], socksClient: g3.socksClient, dialAttempts: 1, prove: fakeProve, socksIsolation: false });
  c3.pool = fakePool;
  ...
  const c4 = new RgoeClient({ secret: "0x01", onion: "a".repeat(56), artifacts: [BUILTIN], socksClient: g4.socksClient, dialAttempts: 1, prove: fakeProve, socksIsolation: false });
  c4.pool = fakePool;
