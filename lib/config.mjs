    ...
    conditional(env, errors) {
      ...
      if ((hasDir || hasBoot) && !hasSigner && !hasPin) {
        errors.push({
          ...
          problem:
            "required for directory/bootnode discovery: the pinned directory signer has no default (SHADE_TREE_DIRECTORY / SHADE_TREE_BOOTNODE_ONION set without it disables discovery)",
        });
      }
    },
  },
