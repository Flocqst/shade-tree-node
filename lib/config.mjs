...
// RGOE_ADMIT: comma list drawn from {invited, staked, paid} (gateway admission policy, T-FEAT-9).
export function isAdmitList(v) {
  if (typeof v !== "string") return false;
  const parts = v.split(",").map((x) => x.trim().toLowerCase()).filter(Boolean);
  return parts.length > 0 && parts.every((p) => p === "invited" || p === "staked" || p === "paid");
}
// RGOE_PAY_PROTOCOLS: comma list drawn from {x402, mpp} (registrar rails, T-FEAT-9).
export function isPayProtocolList(v) {
  if (typeof v !== "string") return false;
  const parts = v.split(",").map((x) => x.trim().toLowerCase()).filter(Boolean);
  return parts.length > 0 && parts.every((p) => p === "x402" || p === "mpp");
}
// RGOE_LEAF_SOURCE: auto | invited | staked | paid (client leaf-source pin, T-FEAT-9).
export function isLeafSource(v) {
  return typeof v === "string" && ["auto", "invited", "staked", "paid"].includes(v.trim().toLowerCase());
}
// A boolean flag: 0/1/true/false/yes/no/on/off (RGOE_MAX_ANON).
...
const P_ROOTS = "must be a comma-separated list drawn from: static, onchain (DEPRECATED alias of RGOE_ADMIT)";
...
export const ROLE_SPECS = {
  ...
  gateway: {
    ...
    optional: [
      ...
      ["RGOE_ADMIT", isAdmitList, P_ADMIT],                // T-FEAT-9: invited[,staked][,paid] (default invited = max-anon)
      ["RGOE_ROOTS", isRootSourceList, P_ROOTS],           // T-FEAT-7 spelling, deprecated alias (static->invited, onchain->staked+paid)
      ["RGOE_PAY_PROTOCOLS", isPayProtocolList, P_PAYPROTO], // T-FEAT-9: registrar / advert rails (heartbeat shares the gateway role)
  ...
  client: {
    ...
    optional: [
      ...
      ["RGOE_RPC_URL", isUrl, P_URL],
      ["RGOE_LEAF_SOURCE", isLeafSource, P_LEAFSRC],   // T-FEAT-9: pin the set to prove from
      ["RGOE_MAX_ANON", isBoolFlag, P_BOOL],           // T-FEAT-9: invited-only gateways
