...
//
//   RGOE_LOG_FORMAT = text | json          (default: text — today's human-readable lines)
//   RGOE_LOG_LEVEL  = debug|info|warn|error (default: info)
//
...
function threshold() {
  return LEVELS[String(process.env.RGOE_LOG_LEVEL || "info").toLowerCase()] ?? LEVELS.info;
}
function jsonMode() {
  return String(process.env.RGOE_LOG_FORMAT || "text").toLowerCase() === "json";
}
