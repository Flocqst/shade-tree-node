...
async function main() {
  ...
  // blockTag() reorg-safety branch. RGOE_CONFIRMATIONS is captured at module-eval time
  ...
  console.log("\nOnchainStakeVerifier reads stake at a confirmed depth (head-N) when RGOE_CONFIRMATIONS>0:");
  {
    const realFetch = globalThis.fetch;
    const savedConf = process.env.RGOE_CONFIRMATIONS;
    const CONTRACT = "0xabcabcabcabcabcabcabcabcabcabcabcabcabca";
    const addr = "0x1111111111111111111111111111111111111111";
    try {
      process.env.RGOE_CONFIRMATIONS = "6";
      ...
      if (savedConf === undefined) delete process.env.RGOE_CONFIRMATIONS; else process.env.RGOE_CONFIRMATIONS = savedConf;
    }
  }

  console.log("\nOnchainStakeVerifier reads at 'latest' when RGOE_CONFIRMATIONS unset/0 (byte-identical to today):");
  {
    const realFetch = globalThis.fetch;
    const savedConf = process.env.RGOE_CONFIRMATIONS;
    const CONTRACT = "0xabcabcabcabcabcabcabcabcabcabcabcabcabca";
    const addr = "0x1111111111111111111111111111111111111111";
    try {
      delete process.env.RGOE_CONFIRMATIONS;
  ...
  console.log("\nOnchainStakeVerifier head-N boundary + hex encoding (depth deeper than head clamps to 0x0):");
  {
    ...
    try {
      ...
      process.env.RGOE_CONFIRMATIONS = "100";
