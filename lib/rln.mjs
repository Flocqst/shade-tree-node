...
// The static (PoC / friends) member list. RGOE_MEMBERS_FILE overrides the path (T-FEAT-7: a
...
export const MEMBERS_PATH = process.env.RGOE_MEMBERS_FILE || join(HERE, "..", "group", "members.json");
...
//   gateway: RGOE_ZK_ARTIFACTS  {id -> vkey}   (+ RGOE_ZK_ARTIFACT_LEGACY for field-less envelopes)
//   client:  RGOE_ZK_PROVER_ARTIFACTS  [{id, wasm, zkey}] newest first
...
// proof); it only widens the slash path to non-default tiers. RGOE_TIERS="8,32" (ascending,
// distinct, each 1..MAX_LIMIT); the default tier K_SLOTS is always included. Default: [K_SLOTS].
export function parseTiers(spec = process.env.RGOE_TIERS, k = K_SLOTS) {
  ...
  if (raw !== "") {
    for (const part of raw.split(",")) {
      const t = part.trim();
      if (t === "") continue;
      if (!/^[0-9]+$/.test(t)) throw new Error(`RGOE_TIERS: bad tier "${t.slice(0, 16)}" (integers only)`);
...
export async function loadGroupOnchain(rootProvider) {
  ...
  return { recentRoots: (r.roots || []).map(String), perSource: r.perSource, errors: r.errors, leafCount: r.leafCount };
}
...
// the process-wide set from RGOE_ZK_ARTIFACTS (getArtifactSet). Rejections from 3b also carry
