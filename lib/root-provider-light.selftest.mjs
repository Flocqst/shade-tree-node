...
async function main() {
  ...
  // ---- 8. trustedStateRoot injection hook is honored (Helios sidecar seam) ----------------
  {
    const built = buildProofResponse(GOLDEN_ROOT);
    const prev = global.fetch;
    global.fetch = async (_url, opts) => {
      const req = JSON.parse(opts.body);
      // NOTE: no eth_getBlockByNumber here except for confirmations; proof served directly.
      let result = null;
      if (req.method === "eth_getProof") {
        ...
        result = { number: "0x64", stateRoot: "0x" + "de".repeat(32) }; // WRONG state root; must be ignored
      }
      return { json: async () => ({ jsonrpc: "2.0", id: req.id, result }) };
    };
    const p = LightClientRootProvider({
      contract: CONTRACT,
      rpcUrl: "http://stub",
      trustedStateRoot: async () => ({ stateRoot: built.stateRoot, number: 100 }),
    });
    const res = await p.currentRoots();
    ok(res.roots[0] === GOLDEN_ROOT.toString(), "trustedStateRoot hook anchors the proof (RPC block ignored)");
    global.fetch = prev;
  }
