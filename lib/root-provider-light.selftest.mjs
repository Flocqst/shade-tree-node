...
async function main() {
  ...
  // real Helios implementation of this hook end to end.
  {
    ...
    ok(stale.stale === true && stale.roots[0] === GOLDEN_ROOT.toString() && /stateRoot mismatch at block 100: RPC \(http:\/\/stub\) claims 0xdede/.test(stale.error) && stale.error.includes(built.stateRoot),
      "RPC header stateRoot != anchor after a good read -> last-known-good root, stale:true, precise mismatch reason");
    const fresh = LightClientRootProvider({ contract: CONTRACT, rpcUrl: "http://stub", heliosRpcUrl: "", trustedStateRoot: async () => ({ stateRoot: built.stateRoot, number: 100 }) });
