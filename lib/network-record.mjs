...
//   bootnode.json  -> SHADE_TREE_BOOTNODE_ONION, SHADE_TREE_DIR_SIGNER, SHADE_TREE_BOOTNODE_ADMISSION
...
export function envDefaultsFromRecords({ dir, contracts, bootnode }) {
  ...
  if (bootnode) {
    ...
    if (!out.SHADE_TREE_BOOTNODE_ONION && bootnode.staticDirectory && bootnode.staticDirectory.path) {
...
export function applyNetworkEnv(env = process.env, { name = env.SHADE_TREE_NETWORK, root } = {}) {
  if (!name) return [];
