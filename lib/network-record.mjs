...
// Serialize / parse the per-contract start-block map RGOE_FROM_BLOCKS: `<addr>=<block>,...`
...
export function parseFromBlocks(spec) {
  ...
  for (const part of String(spec == null ? "" : spec).split(",")) {
    ...
    if (!isEthAddress(addr) || !/^(0x[0-9a-fA-F]+|\d+)$/.test(blk)) throw new Error(`RGOE_FROM_BLOCKS: bad entry "${s.slice(0, 64)}" (expected <0xaddress>=<block>, comma-separated)`);
...
//                     RGOE_PAID_ACCESS_CONTRACT (= paidAccessSet), RGOE_PAY_ASSET (= payAsset.address),
//                     RGOE_REGISTRAR_PORT (= registrar.port)     [T-FEAT-7]
export function envDefaultsFromRecords({ dir, contracts, bootnode }) {
  ...
  if (contracts) {
    ...
    if (paid) out.RGOE_PAID_ACCESS_CONTRACT = paid;
    if (isUrl(contracts.rpcUrl)) out.RGOE_RPC_URL = contracts.rpcUrl;
    if (contracts.payAsset && isEthAddress(contracts.payAsset.address)) out.RGOE_PAY_ASSET = contracts.payAsset.address;
    if (contracts.registrar && Number.isInteger(contracts.registrar.port)) out.RGOE_REGISTRAR_PORT = String(contracts.registrar.port);
    // eth_getLogs start blocks (lib/root-provider.mjs): the MIN deploy block of the contracts this
    // record configures (RGOE_FROM_BLOCK) plus the exact per-contract map (RGOE_FROM_BLOCKS), so
    ...
    if (blocks.length) {
      out.RGOE_FROM_BLOCK = "0x" + Math.min(...blocks).toString(16);
      out.RGOE_FROM_BLOCKS = formatFromBlocks(fromBlocks);
    }
  }
  return out;
}
...
export function applyNetworkEnv(env = process.env, { name = env.RGOE_NETWORK, root } = {}) {
  ...
  // RGOE_FROM_BLOCK / RGOE_FROM_BLOCKS are ONE setting with two spellings: an operator who pins
  // either explicitly has decided where scans start, so neither is filled from the record then
  // (otherwise a derived per-contract map would silently out-rank an explicit RGOE_FROM_BLOCK).
  const fromPinned = !unset("RGOE_FROM_BLOCK") || !unset("RGOE_FROM_BLOCKS");
  for (const [k, v] of Object.entries(defaults)) {
    if (fromPinned && (k === "RGOE_FROM_BLOCK" || k === "RGOE_FROM_BLOCKS")) continue;
...
// throws: the deploy block of `address` from network/<name>/contracts.json (name = RGOE_NETWORK by
// default), else from ANY committed network record that lists the address (a fleet unit that pins
// RGOE_GROUP_CONTRACT / RGOE_PAID_ACCESS_CONTRACT explicitly, without RGOE_NETWORK, still gets a
...
export function deployBlockForContract(address, { name, env = process.env, root = NETWORK_ROOT } = {}) {
  ...
  const names = [];
  const preferred = name || (env && env.RGOE_NETWORK);
