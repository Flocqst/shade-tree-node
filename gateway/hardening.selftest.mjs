...
//   - RGOE_MAX_CONNS: the (max+1)th concurrent socket is refused `too-many-connections`
//     BEFORE any envelope read; closing one admits the next (release on close)
//   - RGOE_MAX_CONNS_PER_NULLIFIER: the (max+1)th tunnel on ONE nullifier is refused
...
process.env.RGOE_EGRESS_ALLOW = "127.0.0.1:*";
...
const envelopeFor = (nullifier, port, x = "1") => JSON.stringify({ v: 3, target: `127.0.0.1:${port}`, nullifier, share: { x, y: "2" }, nonce: "n" }) + "\n";
...
// ---- slow-loris on the envelope read ----------------------------------------
{
  ...
  await test("slow-loris SILENCE: connect + never send => cut at the deadline, reason envelope-timeout", async () => {
    const before = metricValue("rgoe_gateway_requests_total", { result: "drop", reason: "envelope-timeout" });
    const t0 = Date.now();
    ...
    assert.equal(metricValue("rgoe_gateway_requests_total", { result: "drop", reason: "envelope-timeout" }), before + 1, "drop reason envelope-timeout counted once");
  });
  ...
  await test("slow-loris DRIBBLE: a byte every 20ms with no newline => cut at the SAME absolute deadline", async () => {
    ...
    assert.equal(metricValue("rgoe_gateway_requests_total", { result: "drop", reason: "envelope-timeout" }), before + 1);
  });
  ...
  await test("envelope size cap => envelope-too-large; close-before-newline => bad-envelope (both counted, socket released)", async () => {
    const tooBig = metricValue("rgoe_gateway_requests_total", { result: "drop", reason: "envelope-too-large" });
    const badEnv = metricValue("rgoe_gateway_requests_total", { result: "drop", reason: "bad-envelope" });
    ...
    assert.equal(metricValue("rgoe_gateway_requests_total", { result: "drop", reason: "envelope-too-large" }), tooBig + 1);
    ...
    assert.equal(metricValue("rgoe_gateway_requests_total", { result: "drop", reason: "bad-envelope" }), badEnv + 1);
  });
...
// ---- idle timeout on the established relay ----------------------------------
{
  ...
  await test("established relay: bytes flow both ways, then idle in BOTH directions => closed, idle-timeout counted", async () => {
    const before = metricValue("rgoe_gateway_tunnel_closes_total", { reason: "idle-timeout" });
    const c = await dial(port);
    ...
    assert.equal(metricValue("rgoe_gateway_tunnel_closes_total", { reason: "idle-timeout" }), before + 1, "idle-timeout counted once");
  });
  ...
  await test("a black-holed upstream connect (never completes, never errors) is bounded by the idle timeout", async () => {
    ...
    const before = metricValue("rgoe_gateway_requests_total", { result: "drop", reason: "upstream-timeout" });
    ...
    assert.equal(metricValue("rgoe_gateway_requests_total", { result: "drop", reason: "upstream-timeout" }), before + 1);
...
// ---- concurrent-connection caps ---------------------------------------------
{
  ...
  await test("RGOE_MAX_CONNS: the 4th concurrent socket is refused too-many-connections BEFORE any envelope; closing one admits the next", async () => {
    const before = metricValue("rgoe_gateway_requests_total", { result: "drop", reason: "too-many-connections" });
    ...
    assert.equal(metricValue("rgoe_gateway_requests_total", { result: "drop", reason: "too-many-connections" }), before + 1);
  ...
  await test("RGOE_MAX_CONNS_PER_NULLIFIER: 3rd tunnel on ONE nullifier refused nullifier-conn-limit; other nullifiers fine; spent-set still saw the share; released on close", async () => {
    const before = metricValue("rgoe_gateway_requests_total", { result: "drop", reason: "nullifier-conn-limit" });
    ...
    assert.equal(metricValue("rgoe_gateway_requests_total", { result: "drop", reason: "nullifier-conn-limit" }), before + 1);
