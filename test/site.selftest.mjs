...
check("landing uses one compact keyboard-scrollable protocol figure", /<figure class="how-figure" role="region" aria-label="Shade Tree protocol diagram" tabindex="0">[\s\S]*shade-tree-readme\.svg[\s\S]*<\/figure>/.test(landing) && /<code tabindex="0" aria-label="v0\.5\.0 live binary installation command">/.test(landing) && (landing.match(/class="how-figure"/g) || []).length === 1 && !/class="how-mechanism"/.test(landing));
check("landing has role-specific copyable installs", /<p class="step-label">Install the live Rust binary<\/p>[\s\S]*?<div class="command agent-install"/.test(landing) && /--proto '=https' --proto-redir '=https'/.test(landing) && /scripts\/install\.sh \| SHADE_TREE_VERSION=v0\.5\.0 sh/.test(landing) && /selects a published <code>-live<\/code> asset and verifies its checksum/.test(landing) && /data-copy="git clone https:\/\/github\.com\/dmarzzz\/shade-tree-node\.git &amp;&amp; cd shade-tree-node &amp;&amp; npm ci &amp;&amp; npm link"/.test(landing) && /data-copy="shade-tree join node"/.test(landing) && /aria-label="Copy v0\.5\.0 live binary installation command"/.test(landing) && /aria-label="Copy node installation commands"/.test(landing) && !/npm install --global git\+https:\/\/github\.com\/dmarzzz\/shade-tree-node\.git/.test(landing));
...
check("identity, Proxy token, and Proxy launch remain separate copy actions", /public-leaf\.txt/.test(identityEnrollCopy) && /register-member/.test(identityEnrollCopy) && !/proxy-token/.test(identityEnrollCopy) && /shade-tree proxy-token/.test(proxyTokenCopy) && !/--bootnode-onion/.test(proxyTokenCopy) && /--identity identity\.json/.test(proxyStartCopy) && !/--members members\.json/.test(proxyStartCopy) && /--listen 127\.0\.0\.1:8118/.test(proxyStartCopy));
