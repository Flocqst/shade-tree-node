...
check("landing and README embed responsive two-plane path graphics", /<picture\b[^>]*>[\s\S]*?media="\(max-width: 560px\)"[\s\S]*?srcset="\/fig\/shade-tree-path-mobile\.svg"[^>]+width="720" height="1710"[\s\S]*?<img src="\/fig\/shade-tree-path\.svg"/.test(landing) && /<picture>[\s\S]*?srcset="docs\/post\/fig\/shade-tree-path-mobile\.svg"[\s\S]*?<img src="docs\/post\/fig\/shade-tree-path\.svg"/.test(readme));
...
check("home scene is an irregular canopy map with one bounded pixel route", /targetCount = mobile \? 18 : 29/.test(landingScene) && /crowded = trees\.some/.test(landingScene) && /SphereGeometry\(1, 7, 5\)/.test(landingScene) && /CatmullRomCurve3/.test(landingScene) && /segmentCount = mobile \? 30 : 44/.test(landingScene) && /const head = new THREE\.Mesh/.test(landingScene) && /positionPacket/.test(landingScene));
check("home scene uses high-angle framing, a copy clearing, and one orchestrated signal", /inCopyClearing/.test(landingScene) && /new THREE\.ShapeGeometry\(shape\)/.test(landingScene) && /new THREE\.Fog\(NIGHT, 35, 60\)/.test(landingScene) && /new THREE\.Vector3\(mobile \? 8 : 14, mobile \? 31 : 28, mobile \? 18 : 18\)/.test(landingScene) && /viewHeight = mobile \? 28 : 22/.test(landingScene) && /new THREE\.RingGeometry/.test(landingScene) && /const halo = new THREE\.Mesh/.test(landingScene) && /renderer\.shadowMap\.enabled = !mobile/.test(landingScene));
