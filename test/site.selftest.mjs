...
const handoffBrief = landing.match(/class="command command-brief"[\s\S]*?<code>([\s\S]*?)<\/code>/)?.[1] || "";
...
check("landing hero jumps straight to both role starts", /class="solid-action" href="#proxy">Add to an agent/.test(landing) && /class="line-action" href="#node">Run a node/.test(landing) && /class="solid-action compact" href="\/agent\/">Agent guide/.test(landing) && /class="solid-action compact" href="\/operator\/">Operator guide/.test(landing));
check("landing separates the reputation gate from the network path", /<h3>Reputation gate<\/h3>/.test(landing) && /admission policy, not the proof/.test(landing) && /Groth16 RLN/.test(landing) && /rate-commitment leaf/.test(landing) && /epoch-scoped nullifiers/.test(landing) && /<h3>Network path<\/h3>/.test(landing) && /signed Canopy/.test(landing) && /target-bound CONNECT tunnel through Tor/.test(landing) && /Elder Tree stays outside the traffic path/.test(landing));
...
check("home scene uses pulled-back framing, a route glade, and a restrained signal thread", /xMin = mobile \? -5\.6 : -15\.4/.test(landingScene) && /Preserve a slim diagonal glade/.test(landingScene) && /new THREE\.Fog\(NIGHT, 22, 46\)/.test(landingScene) && /viewHeight = \(mobile \? 16\.8 : 16\.4\) \+ narrowBoost/.test(landingScene) && /new THREE\.Line\(threadGeometry, threadMaterial\)/.test(landingScene) && /const halos = new THREE\.InstancedMesh/.test(landingScene) && /renderer\.shadowMap\.enabled = !mobile/.test(landingScene));
...
check("clipboard failure selects the visible command and cleans up the helper", /finally\s*\{\s*helper\.remove\(\)/.test(loader) && /range\.selectNodeContents\(visibleCode\)/.test(loader) && /Command selected\. Press Control\+C or Command\+C to copy\./.test(loader));
...
check("agent install choices stack before their fixed track minima can clip", /@media \(max-width: 1100px\)\s*\{[\s\S]*?\.install-options\s*\{\s*grid-template-columns:\s*1fr/.test(landingCss));
