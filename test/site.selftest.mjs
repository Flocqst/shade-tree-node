...
check("home scene uses high-angle framing, a feathered shade field, and a continuous sunbeam", /inCopyClearing/.test(landingScene) && /new THREE\.PlaneGeometry\(mobile \? 17 : 34, mobile \? 27 : 23\)/.test(landingScene) && /new THREE\.ShaderMaterial/.test(landingScene) && /smoothstep\(0\.56 \+ edge, 0\.99 \+ edge/.test(landingScene) && /new THREE\.PlaneGeometry\(mobile \? 1\.1 : 1\.7, mobile \? 44 : 64\)/.test(landingScene) && !/new THREE\.ShapeGeometry\(shape\)/.test(landingScene) && /new THREE\.Fog\(NIGHT, 35, 60\)/.test(landingScene) && /new THREE\.Vector3\(mobile \? 8 : 14, mobile \? 31 : 28, mobile \? 18 : 18\)/.test(landingScene) && /viewHeight = mobile \? 28 : 22/.test(landingScene) && /renderer\.shadowMap\.enabled = !mobile/.test(landingScene));
