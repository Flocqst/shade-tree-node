...
check("Grove keeps the restored detail table in the historical desktop and mobile hierarchy", /grid-template-columns:\s*minmax\(14rem, 0\.78fr\) minmax\(28rem, 1\.65fr\) minmax\(14rem, 0\.82fr\)/.test(groveCss) && /@media \(max-width: 1050px\)[\s\S]*?\.network-visual\s*\{\s*grid-row:\s*span 2;\s*\}[\s\S]*?\.provenance-panel\s*\{\s*grid-column:\s*1;\s*\}/.test(groveCss) && /@media \(max-width: 760px\)[\s\S]*?\.network-visual\s*\{[\s\S]*?grid-row:\s*auto;[\s\S]*?\}[\s\S]*?\.provenance-panel\s*\{\s*grid-column:\s*auto;\s*width:\s*100%;\s*\}/.test(groveCss));
