...
check("landing sections use restrained full-width grove clearing bands", /--clearing-about:\s*#0a1912/.test(landingCss) && /--clearing-agent:\s*#0e2017/.test(landingCss) && /--clearing-node:\s*#0b1c14/.test(landingCss) && /--clearing-how:\s*#0d1f16/.test(landingCss) && /\.home-main\s*\{[\s\S]*?width:\s*100%/.test(landingCss) && /\.about-panel\s*\{[\s\S]*?background:\s*var\(--clearing-about\)/.test(landingCss) && /\.role:first-child\s*\{\s*background:\s*var\(--clearing-agent\)/.test(landingCss) && /\.role:last-child\s*\{\s*background:\s*var\(--clearing-node\)/.test(landingCss) && /\.how-panel\s*\{[\s\S]*?background:\s*var\(--clearing-how\)/.test(landingCss) && !/border-radius[^;]*;[\s\S]{0,120}clearing|clearing[^}]*border-radius/.test(landingCss));
...
check("Grove introduces the aggregate view in one plain sentence", /A privacy-preserving view of the Grove, built from signed aggregate reports without publishing node identities or locations\./.test(grovePage) && !/Census field|signed census of the pre-v4 research fleet|Census arcs are not traffic/.test(grovePage));
...
check("compact navigation and controls keep full touch targets", /\.wordmark\s*\{[\s\S]*?min-height:\s*2\.75rem/.test(landingCss) && /\.nav-links a\s*\{[\s\S]*?min-width:\s*2\.75rem;[\s\S]*?min-height:\s*2\.75rem/.test(landingCss) && /\.copy-command\s*\{[\s\S]*?min-width:\s*7\.6rem;[\s\S]*?min-height:\s*2\.75rem/.test(landingCss) && /\.site-footer a\s*\{[\s\S]*?min-width:\s*2\.75rem;[\s\S]*?min-height:\s*2\.75rem/.test(landingCss) && /\.ledger-links a\s*\{[\s\S]*?min-height:\s*2\.75rem/.test(groveCss));
check("Grove privacy ledger names the exact published boundary", /<section class="public-ledger" aria-labelledby="public-ledger-title">[\s\S]*?<h2 id="public-ledger-title">What’s public<\/h2>\s*<p>The Grove publishes signed totals, never a list of nodes\.<\/p>[\s\S]*?<h3>Published<\/h3>\s*<ul>\s*<li>Current node count<\/li>\s*<li>Reporting coverage and freshness<\/li>\s*<li>24-hour count history<\/li>\s*<li>Publication signature<\/li>[\s\S]*?<h3>Never published<\/h3>\s*<ul>\s*<li>Node identities<\/li>\s*<li>Locations, IPs, or networks<\/li>\s*<li>Traffic or destinations<\/li>/.test(grovePage));
check("Grove keeps plain specification links beneath the privacy ledger", /<\/div>\s*<nav class="ledger-links" aria-label="Technical specifications">\s*<a href="https:\/\/github\.com\/dmarzzz\/shade-tree-node\/blob\/main\/specs\/data-api\.md">Data API<\/a>\s*<a href="https:\/\/github\.com\/dmarzzz\/shade-tree-node\/blob\/main\/specs\/protocol\.md">Protocol specification<\/a>\s*<\/nav>\s*<\/section>/.test(grovePage) && !/ledger-links[\s\S]*?↗/.test(grovePage));
check("Grove removes abstract boundary and attestation copy", !/A view with boundaries|Signed view|Observer attests|Browser verifies|A scheduled observer/.test(grovePage) && /<span>Snapshot<\/span>/.test(grovePage) && /<dt>Refresh target<\/dt><dd data-snapshot-cadence>15 min<\/dd>/.test(grovePage));
check("Grove renders the privacy boundary as a quiet responsive ledger", /\.ledger-intro\s*\{[\s\S]*?border-bottom:\s*1px solid var\(--line\)/.test(groveCss) && /\.ledger-columns > div \+ div\s*\{[\s\S]*?border-left:\s*1px solid var\(--line\)/.test(groveCss) && /@media \(max-width: 760px\)[\s\S]*?\.ledger-intro,[\s\S]*?\.ledger-columns\s*\{\s*grid-template-columns:\s*1fr;[\s\S]*?\.ledger-columns > div \+ div\s*\{[\s\S]*?border-top:\s*1px solid var\(--line\);[\s\S]*?border-left:\s*0/.test(groveCss) && !/\.ledger-columns > div\s*\{[^}]*background|\.ledger-columns li::before|\.ledger-columns h3\s*\{[^}]*text-transform/.test(groveCss));
...
check("stacked cameras reserve a text-safe clearing without changing desktop", /tabletClearing = mobile && width > 600/.test(landingScene) && /verticalOffset = mobile \? \(tabletClearing \? 9\.5 : 8\.5\) : 0/.test(landingScene) && /camera\.top = viewHeight \/ 2 \+ verticalOffset/.test(landingScene) && /camera\.bottom = -viewHeight \/ 2 \+ verticalOffset/.test(landingScene) && /@media \(max-width: 900px\)[\s\S]*?\.forest-fallback \{ inset: 52% -8% -6% 7%; \}/.test(landingCss));
