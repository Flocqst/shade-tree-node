...
check("landing uses one compact keyboard-scrollable protocol figure", /<figure class="how-figure" role="region" aria-label="Shade Tree protocol diagram" tabindex="0">[\s\S]*shade-tree-readme\.svg[\s\S]*<\/figure>/.test(landing) && /<code tabindex="0" aria-label="v0\.4\.1 live binary installation command">/.test(landing) && (landing.match(/class="how-figure"/g) || []).length === 1 && !/class="how-mechanism"/.test(landing));
check("landing has role-specific copyable installs", /<p class="step-label">Install the live Rust binary<\/p>[\s\S]*?<div class="command agent-install"/.test(landing) && /--proto '=https' --proto-redir '=https'/.test(landing) && /scripts\/install\.sh \| SHADE_TREE_VERSION=v0\.4\.1 sh/.test(landing) && /selects a published <code>-live<\/code> asset and verifies its checksum/.test(landing) && /data-copy="git clone https:\/\/github\.com\/dmarzzz\/shade-tree-node\.git &amp;&amp; cd shade-tree-node &amp;&amp; npm ci &amp;&amp; npm link"/.test(landing) && /data-copy="shade-tree join node"/.test(landing) && /aria-label="Copy v0\.4\.1 live binary installation command"/.test(landing) && /aria-label="Copy node installation commands"/.test(landing) && !/npm install --global git\+https:\/\/github\.com\/dmarzzz\/shade-tree-node\.git/.test(landing));
...
check("landing provides a compact safe handoff for existing agents", /<p class="step-label">Agent setup<\/p>/.test(landing) && /data-copy="Set up Shade Tree Grove for this agent by following https:\/\/github\.com\/dmarzzz\/shade-tree-node\/blob\/main\/docs\/AGENT\.md/.test(landing) && /v0\.4\.1 Rust -live binary/.test(landing) && /embedded Arti needs neither Node\.js nor system Tor/.test(landing) && /bundled v4 discovery default/.test(landing) && /No public admission profile exists/.test(landing) && /Keep identity\.json and proxy-token\.txt private/.test(landing) && /GitHub Discussions/.test(landing) && /data-copy-target="agent-setup-task" data-copy-noun="Setup brief"/.test(landing) && /aria-label="Copy Shade Tree Grove setup brief for an AI agent"/.test(landing) && /<code id="agent-setup-task" class="copy-fallback" hidden>/.test(landing));
...
check("agent page gives the complete Rust live-binary integration path", /--proto '=https' --proto-redir '=https'/.test(agentPage) && /scripts\/install\.sh/.test(agentPage) && /SHADE_TREE_VERSION=v0\.4\.1 sh/.test(agentPage) && /downloads the matching v0\.4\.1 <code>-live<\/code> binary and checksum/.test(agentPage) && /shade-tree enroll --limit/.test(agentPage) && /shade-tree proxy-token/.test(agentPage) && /bundled current-v4 Elder and signer/.test(agentPage) && /--identity identity\.json/.test(agentPage) && /--members members\.json/.test(agentPage) && /--listen 127\.0\.0\.1:8118/.test(agentPage) && !/--secret/.test(agentPage) && /shade-tree run -- your-agent/.test(agentPage) && /replace <code>your-agent<\/code> with <code>hermes<\/code>/.test(agentPage) && /does not include public admission or membership/.test(agentPage) && /needs neither Node\.js nor a system Tor daemon/.test(agentPage) && !/npm install --global/.test(agentPage));
...
const proxyStartCopy = agentCopies.find((command) => command.includes("--identity identity.json")) || "";
