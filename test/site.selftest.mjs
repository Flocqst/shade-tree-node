...
check("Grove removes the snapshot provenance panel without leaving a layout column", !/A view with boundaries|Signed view|Observer attests|Browser verifies|A scheduled observer|provenance-panel|Snapshot provenance|data-snapshot-state|data-view-time|data-network|data-snapshot-cadence/.test(grovePage) && /grid-template-columns:\s*minmax\(14rem, 25rem\) minmax\(28rem, 43rem\)/.test(groveCss) && !/\.provenance-(?:panel|head)/.test(groveCss));
...
check("Grove removes globe overlays without blocking pinch zoom", !/touch-action\s*:|\.scene-key|\.elder-label|\.network-caption/.test(groveCss) && /\.snapshot-state\s*\{[\s\S]*?font-size:\s*0\.75rem/.test(groveCss));
...
check("Grove dashboard stays inside the signed aggregate contract", ["data-node-count", "data-view-age", "data-node-hours", "data-history-low", "data-history-high", "data-history-samples", "data-history-coverage"].every((field) => grovePage.includes(field)) && !/data-change|Observed change|sessions|throughput|latency|bandwidth|success rate|reputation band/i.test(grovePage));
