...
check("fresh signed snapshots are visibly verified", fresh.viewState === "Network snapshot verified · 12 min ago" && !fresh.stale);
...
check("refresh errors preserve and label the last verified live view", delayed.viewState === "Update unavailable · last verified 12 min ago" && !delayed.stale);
const delayedStale = snapshotFreshness(snapshot, { now: observedMs + 46 * 60_000, refreshFailed: true });
check("a delayed live view becomes stale as its clock advances", delayedStale.stale);

const reference = snapshotFreshness(snapshot, { now: observedMs + 12 * 60_000, bundled: true });
check("the deploy-bundled fallback is always labeled as a reference", reference.viewState === "Signed pre-v4 reference · 12 min ago" && reference.stale);
